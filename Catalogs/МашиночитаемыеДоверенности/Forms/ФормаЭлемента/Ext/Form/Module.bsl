		...
		Если ЗначениеЗаполнено(ИНН) Тогда
			...
			Если ТаблицаДляПроверкиДублей.НайтиСтроки(СтруктураОтбора).Количество() > 0 Тогда
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки, Строка.Доверитель, НСтр(
					...
					|en = 'указано несколько доверителей с одним ИНН.'"));
	...
	Для Каждого Строка Из Объект.Представители Цикл
		...
		Если ЗначениеЗаполнено(ИНН) Тогда
			...
			Если ТаблицаДляПроверкиДублей.НайтиСтроки(СтруктураОтбора).Количество() > 0 Тогда
				...
				|en = 'указано несколько представителей с одним ИНН.'"));
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки,, Поле,, Отказ);
			Иначе
				НоваяСтрока = ТаблицаДляПроверкиДублей.Добавить();
				НоваяСтрока.ИНН = ИНН;
			...
			Если ТаблицаДляПроверкиДублей.НайтиСтроки(СтруктураОтбора).Количество() > 0 Тогда
				...
				|en = 'указан представитель и доверитель с одним и тем же ИНН.'"));
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки,, Поле,, Отказ);
		...
		Форма.Элементы.ОписаниеСостояния.Заголовок = МодульСтроковыеФункции.ФорматированнаяСтрока(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '<span style=""color: %1"">Отменена в реестре ФНС</span>';
					|en = '<span style=""color: %1"">Отменена в реестре ФНС</span>'"), "ПоясняющийТекст"));
		Возврат;
	КонецЕсли;
