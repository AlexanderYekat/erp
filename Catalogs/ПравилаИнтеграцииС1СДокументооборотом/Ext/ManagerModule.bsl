...
Функция РеквизитыОбъектаДО(ТипОбъектаДО, Параметры = Неопределено, НастройкиВида = Неопределено,
			...
			Исключение // Возможно, этого вида документа уже нет в ДО.
				...
				ИнтеграцияС1СДокументооборотБазоваяФункциональность.ЗаписатьПредупреждение(
					СтрШаблон(НСтр("ru = 'Не найден в 1С:Документообороте ранее существовавший вид документа %1';
									|en = 'Не найден в 1С:Документообороте ранее существовавший вид документа %1'"),
	...
	Если ТипОбъектаДО = "DMIncomingDocument" Тогда
		...
		Если ПолучитьВсе Или НастройкиТипа.organizationEnabled = Истина Тогда
			ДобавитьРеквизит(Реквизиты, "organization", НСтр("ru = 'Организация';
															|en = 'Организация'"), "DMOrganization", Истина);
		КонецЕсли;
		...
		Если ПолучитьВсе Или НастройкиТипа.accessLevelEnabled  = Истина Тогда
			ДобавитьРеквизит(Реквизиты, "accessLevel", НСтр("ru = 'Гриф доступа';
															|en = 'Гриф доступа'"), "DMAccessLevel", Истина);
		КонецЕсли;
		...
		Если ПолучитьВсе Или НастройкиТипа.activityMatterEnabled = Истина Тогда
			ДобавитьРеквизит(Реквизиты, "activityMatter", НСтр("ru = 'Вопрос деятельности';
																|en = 'Вопрос деятельности'"), "DMActivityMatter", Истина);
		КонецЕсли;
		...
		ДобавитьРеквизит(Реквизиты, "deliveryMethod",  НСтр("ru = 'Способ получения';
															|en = 'Способ получения'"), "DMDeliveryMethod");
	...
	ИначеЕсли ТипОбъектаДО = "DMOutgoingDocument" Тогда
		...
		Если ПолучитьВсе Или НастройкиТипа.documentTypeEnabled = Истина Тогда
			ДобавитьРеквизит(Реквизиты, "documentType", НСтр("ru = 'Вид документа';
															|en = 'Вид документа'"), "DMOutgoingDocumentType", Истина);
		КонецЕсли;
			
		ДобавитьРеквизит(Реквизиты, "correspondent", НСтр("ru = 'Получатель';
															|en = 'Получатель'"), "DMCorrespondent", Истина);
		...
		ДобавитьРеквизит(Реквизиты, "deliveryMethod", НСтр("ru = 'Способ отправки';
															|en = 'Способ отправки'"), "DMDeliveryMethod");
		ДобавитьРеквизит(Реквизиты, "comment", НСтр("ru = 'Комментарий';
													|en = 'Комментарий'"), "Строка");
		ДобавитьРеквизит(Реквизиты, "responsible", НСтр("ru = 'Ответственный';
														|en = 'Ответственный'"), "DMUser");
	...
	ИначеЕсли ТипОбъектаДО = "DMInternalDocument" Тогда
		...
		Если ПолучитьВсе Или НастройкиТипа.documentTypeEnabled = Истина Тогда
			ДобавитьРеквизит(Реквизиты, "documentType", НСтр("ru = 'Вид документа';
															|en = 'Вид документа'"), "DMInternalDocumentType", Истина);
		КонецЕсли;
		
		ДобавитьРеквизит(Реквизиты, "folder", НСтр("ru = 'Папка';
													|en = 'Папка'"), "DMInternalDocumentFolder", Истина);
		...
		Если ПолучитьВсе Или НастройкиВида = Неопределено Или НастройкиВида.durationEnabled = Истина Тогда
			ДобавитьРеквизит(Реквизиты,
				"beginDate", НСтр("ru = 'Дата начала действия';
									|en = 'Certificate start date'"), "Дата");
			ДобавитьРеквизит(Реквизиты,
				"endDate", НСтр("ru = 'Дата окончания действия';
								|en = 'Certificate end date'"), "Дата");
			ДобавитьРеквизит(Реквизиты,
				"openEnded", НСтр("ru = 'Бессрочный';
									|en = 'Бессрочный'"), "Булево");
			ДобавитьРеквизит(Реквизиты,
				"prolongationProcedure", НСтр("ru = 'Порядок продления';
												|en = 'Порядок продления'"), "DMProlongationProcedure");
		КонецЕсли;
		...
		Если ПолучитьВсе Или (ЯвляетсяЗаявкойНаОплату = Ложь И ВестиУчетСторон) Тогда
			...
			ДобавитьРеквизитТаблицы(Реквизиты,
				"partyRows.rows", "contact", НСтр("ru = 'Контактное лицо';
													|en = 'Контактное лицо'"), "DMPartyRowContact");
	...
	Если ТипОбъектаДО = "DMIncomingDocument"
		...
		Если ПолучитьВсе Или (НастройкиТипа.statusEnabled = Истина И НастройкиТипа.statusChangeEnabled = Истина) Тогда
			...
			ДобавитьРеквизит(Реквизиты, "statusConsideration", НСтр("ru = 'Состояние рассмотрения';
																	|en = 'Состояние рассмотрения'"), "DMDocumentStatus");
			ДобавитьРеквизит(Реквизиты, "statusPerformance", НСтр("ru = 'Состояние исполнения';
