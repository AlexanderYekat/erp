	...
	Если Параметры.НайденыДубли И Не ЕстьПравоНаОбъединениеДублей Тогда
		ТекстПредупреждения = 
			НСтр("ru = 'Найдены похожие люди. Если это дубли, обратитесь к пользователю с правами на объединение дублей.';
				|en = 'Найдены похожие люди. Если это дубли, обратитесь к пользователю с правами на объединение дублей.'");
...
Процедура УстановитьСвойстваНадписиОЛюдяхСНедоступнымиДанными(Параметры)
	...
	Если Параметры.ВозможнаПроверкаПоИНН Или Параметры.ВозможнаПроверкаПоСНИЛС Тогда
		...
		ИначеЕсли Параметры.ВозможнаПроверкаПоСНИЛС Тогда
			ТекстНадписиДополнительнойПроверки = НСтр("ru = ' проверьте уникальность по СНИЛС';
														|en = ' проверьте уникальность по СНИЛС'");
		КонецЕсли;
		ТекстНадписи = Новый ФорматированнаяСтрока(
			ТекстНадписи,
			Символы.ПС,
			Новый ФорматированнаяСтрока(
				НСтр("ru = 'Если среди доступных для просмотра данных нет нужного человека,';
					|en = 'Если среди доступных для просмотра данных нет нужного человека,'"),
	...
	Иначе
		ТекстНадписи = Новый ФорматированнаяСтрока(ТекстНадписи, Символы.ПС, СтрЗаменить(
			...
			|en = 'Если среди доступных для просмотра данных нет нужного человека, обратитесь к администратору 
