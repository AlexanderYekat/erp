...
Функция ПоместитьГрафикиВХранилище()
	...
	ГрафикиДоговора.Вставить("ТаблицаОС", РеквизитыЗаполнения.Выгрузить(,"ОсновноеСредство,ГрафикНачисленияПроцентовВведенВручную"));
...
Процедура ОткрытьГрафикОплатИНачисленийЗавершениеНаСервере(Знач РезультатЗакрытия)
	...
	ТаблицаОС = РеквизитыЗаполнения.Выгрузить(,"НомерСтроки,ОсновноеСредство,ГрафикНачисленияПроцентовВведенВручную");
	ЗаполнитьРеквизитыЗаполненияГрафиков(ТаблицаОС);
...
Процедура ЗаполнитьРеквизитыЗаполненияГрафиков(ТаблицаОС)
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(втДатыОкончания.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаОкончанияАренды,
