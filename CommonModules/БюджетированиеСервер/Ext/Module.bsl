...
Функция ФактическиеДанныеИзИсточников(ИмяСправочникаИсточника, ПараметрыПолученияФакта)
	...
	Если ЭтоОстатки Тогда
		...
		ТаблицаОбластейДанных_Копия = ТаблицаОбластейДанных.СкопироватьКолонки();
		Для Каждого СтрокаТаблицы Из ТаблицаОбластейДанных Цикл
			Периоды = БюджетнаяОтчетностьВыводСервер.ПериодыБюджетногоОтчета(СтрокаТаблицы.НачалоПериода, СтрокаТаблицы.КонецПериода, ПараметрыПолученияФакта.Периодичность);
			Для Каждого НачалоПериода Из Периоды Цикл
				НоваяСтрока = ТаблицаОбластейДанных_Копия.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы, , "Период, НачалоПериода, КонецПериода");
				КонецПериода = БюджетированиеКлиентСервер.ДатаКонцаПериода(НачалоПериода, ПараметрыПолученияФакта.Периодичность);
				Если СтрокаТаблицы.Период = СтрокаТаблицы.НачалоПериода Тогда
					НоваяСтрока.Период = НачалоПериода;
				Иначе
					НоваяСтрока.Период = КонецПериода;
				КонецЕсли;
				НоваяСтрока.НачалоПериода = НачалоПериода;
				НоваяСтрока.КонецПериода = КонецПериода;
			КонецЦикла;
		КонецЦикла;
		ТаблицаОбластейДанных = ТаблицаОбластейДанных_Копия;
		
		ТаблицаДопДанных_Копия = ТаблицаДопДанных.СкопироватьКолонки();
		Для Каждого СтрокаТаблицы Из ТаблицаДопДанных Цикл
			Периоды = БюджетнаяОтчетностьВыводСервер.ПериодыБюджетногоОтчета(СтрокаТаблицы.НачалоПериода, СтрокаТаблицы.КонецПериода, ПараметрыПолученияФакта.Периодичность);
			Для Каждого НачалоПериода Из Периоды Цикл
				НоваяСтрока = ТаблицаДопДанных_Копия.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТаблицы, , "Период, НачалоПериода, КонецПериода");
				КонецПериода = БюджетированиеКлиентСервер.ДатаКонцаПериода(НачалоПериода, ПараметрыПолученияФакта.Периодичность);
				Если СтрокаТаблицы.Период = СтрокаТаблицы.НачалоПериода Тогда
					НоваяСтрока.Период = НачалоПериода;
			...
			КонецЦикла;
		КонецЦикла;
		ТаблицаДопДанных = ТаблицаДопДанных_Копия;
		...
		Если ИсточникДанных.ТипИсточника = ТипыИсточников.Обороты 
			...
			Если ПоТаблицеКорреспонденций Тогда
				...
				ФинансоваяОтчетностьСервер.УстановитьОтбор(ГруппаОтборовИ, "РазделИсточникаДанных",
					...
					ВидСравненияКомпоновкиДанных.Равно);
				ФинансоваяОтчетностьСервер.УстановитьОтбор(ГруппаОтборовИ, "ИмяМакетаИсточникаДанных", ,
