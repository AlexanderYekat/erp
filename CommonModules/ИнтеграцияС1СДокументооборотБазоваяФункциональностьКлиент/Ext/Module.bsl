...
Процедура ПоказатьВопросДаНет(ОписаниеОповещенияОЗавершении, ТекстВопроса,
	...
	Иначе
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Недопустимое значение кнопки по умолчанию: %1';
									|en = 'Invalid value for default button: %1'"), КнопкаПоУмолчанию);
	...
	ИначеЕсли ВыделенныеФайлыКоличество = 1 Тогда
		Если ТекущаяПометкаУдаления = Истина Тогда
			ТекстВопроса = СтрШаблон(НСтр("ru = 'Снять с ""%1"" пометку на удаление?';
											|en = 'Do you want to remove the deletion mark from ""%1""?'"), ВыделенныеФайлы[0].Наименование);
		Иначе
			ТекстВопроса = СтрШаблон(НСтр("ru = 'Пометить ""%1"" на удаление?';
											|en = 'Do you want to mark ""%1"" for deletion?'"), ВыделенныеФайлы[0].Наименование);
		КонецЕсли;
