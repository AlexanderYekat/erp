...
Процедура ЗаполнитьПредставлениеСостоянияПодписей(Подписи) Экспорт
	...
	Для Каждого СтрокаПодписи Из Подписи Цикл 
		...
		Если СтрокаПодписи.ЭтоПодписьПоДоверенности Тогда
			...
			Иначе
				Если Не ПроверкаДоверенностиВыполненаВручную Тогда
					...
					СтрокаПодписи.ПодписьВернаПредставление = НСтр("ru = 'Доверенность не найдена';
		...
		Если ЗначениеЗаполнено(СтрокаПодписи.ДатаПроверкиПодписи) Тогда
			СтрокаПодписи.ПодписьВернаПредставление = 
				?(ПодписьВерна, НСтр("ru = 'Подпись верна';
...
Процедура ЗаписатьОшибкуВизуализацииPDFВЖурналРегистрации(ИнформацияОбОшибке) Экспорт
	...
	Шаблон = НСтр("ru = 'Не удалось визуализировать документ PDF по причине:
		|%1';
		|en = 'Не удалось визуализировать документ PDF по причине:
		|%1'");
