	...
	Если ПустаяСтрока(ЧастичныеДанные.НомерДоверенности) Тогда
		...
		Результат.ТекстОшибки
			= НСтр("ru = 'Не удалось получить данные доверенности в реестре ФНС. Повторите попытку позже';
					|en = 'Cannot get authorization letter data from the Federal Tax Service registry. Please try again later'");
		Возврат Результат;
...
#Область Справочники_КлассификаторПолномочийФНСМЧД003_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: обновление классификатора полномочий ФНС (МЧД 003).';
									|en = '1C:Exchange with counterparties: update the FTS authority classifier (machine-readable authorization letter 003).'");
	Обработчик.ПриоритетыВыполнения = ОбновлениеИнформационнойБазы.ПриоритетыВыполненияОбработчика();
	...
	Обработчик.Комментарий = НСтр("ru = '1С:Обмен с контрагентами: обновление классификатора полномочий ФНС (МЧД 003) (начальное заполнение).';
									|en = '1C:Exchange with counterparties: update the FTS authority classifier (machine-readable authorization letter 003) (initial population).'");
...
Процедура ОбновитьКлассификаторПолномочийФНС_МЧД003(ДанныеКлассификатора) Экспорт
	...
	ВидОперации = НСтр("ru = 'Синхронизация классификатора полномочий ФНС (МЧД 003) из сервиса настроек';
						|en = 'Synchronize the FTS authority classifier (machine-readable authorization letter 003) from the settings service'");
...
Функция ПрочитатьАрхивДоверенности(ДвоичныеДанныеАрхива)
	...
	ШаблонОшибки = НСтр("ru = 'Ошибка при чтении архива: %1';
						|en = 'An error occurred when reading the archive: %1'");
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось распаковать архив машиночитаемой доверенности по причине %1';
				|en = 'Cannot unpack the archive of the machine-readable authorization letter. Reason: %1'"), КраткийТекстОшибки);
		ПодробныйТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействие.ОбработатьОшибку(ВидОперации, ПодробныйТекстОшибки, ТекстОшибки);
				...
				Для Каждого СтрокаТЗНастроек Из ТаблицаНастроек Цикл
					...
					Если СвойствоДокумента = Неопределено Тогда
						...
						ТекстОшибки = СтрШаблон(НСтр("ru = 'Не найдено поле данных: %1';
													|en = 'The data field is not found: %1'"), 
					...
					Если НЕ Успех Тогда
						...
						Если ПустаяСтрока(ТекстОшибки) Тогда
							ТекстОшибки = НСтр("ru = 'Полномочия не подтверждены дополнительными правилами проверки';
												|en = 'The authorities are not confirmed with additional check rules'");
						КонецЕсли;
			...
			Если ЗначениеЗаполнено(СведенияРодительскойМЧД.ДатаОтзыва)
				...
				ДействительнаВРеестре.Ошибка = ДействительнаВРеестре.Ошибка + Символы.ПС
					+ СтрШаблон(НСтр("ru = 'Доверенность отозвана %1';
									|en = 'Authorization letter is revoked %1'"),
			...
			Иначе
				...
				ДействительнаВРеестре.Ошибка = 
					СтрШаблон(НСтр("ru = 'На данный момент ее текущий статус: %1';
									|en = 'The current status of the authorization letter: %1'"),
...
Функция МашиночитаемыеПолномочияДоверенности(Доверенность)
	...
	Иначе
		...
		|en = 'The authority check completed with an error.
		|This metadata type does not support getting authority data.'");
...
Функция ТекстДекорацииИнформацииОДополнительнойПроверкеПолномочий() Экспорт
	Возврат НСтр("ru = 'Дополнительные правила проверки полномочий позволяют расширить перечень разрешенных действий, которые определены автоматическими правилами.';
				|en = 'Additional authority check rules allow you to expand the list of permitted actions that are defined by automatic rules.'");
КонецФункции
