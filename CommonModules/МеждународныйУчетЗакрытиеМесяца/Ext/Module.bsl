...
Функция НевыполненныеОтложенныеЗадания(СтатусОтражения, ДатаОтражения, Организация, Документ, Подробно = Ложь) Экспорт
	...
	Для НомерРезультата = 1 По КоличествоРезультатов Цикл
		ОбратныйИндекс = КоличествоРезультатов - НомерРезультата;
		Если Результаты[ОбратныйИндекс].Пустой() Тогда
			НевыполненныеЗадания.Удалить(ОбратныйИндекс);
		ИначеЕсли Подробно Тогда
			Выборка = Результаты[ОбратныйИндекс].Выбрать();
			Выборка.Следующий();
			НевыполненныеЗадания[ОбратныйИндекс] = СтрШаблон(
				НСтр("ru = '%1 с %2.';
					|en = '%1 from %2.'"),
				НевыполненныеЗадания[ОбратныйИндекс],
				Формат(Выборка.Период, НСтр("ru = 'ДФ=MM.yyyy;';
											|en = 'DF=MM/yyyy'")));
		Иначе
			НевыполненныеЗадания[ОбратныйИндекс] = НевыполненныеЗадания[ОбратныйИндекс] + ".";
		КонецЕсли;
	КонецЦикла;
