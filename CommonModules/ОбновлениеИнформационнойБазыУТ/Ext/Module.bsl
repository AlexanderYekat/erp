...
#Область СоздатьЭлементыСтавкиНДСДляМежценовойРазницы
	...
	Обработчик.Комментарий = НСтр("ru = 'Создает новые элементы справочника ""Ставки НДС"" для типа налогообложения ""Облагается НДС с межценовой разницы""';
									|en = 'Creates new ""VAT rates"" catalog items for the ""Subject to VAT from price difference"" taxation type'");
...
#Область УстановитьКонстантуИспользоватьОтветственноеХранениеСобственнымиОрганизациями
	...
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает значение константы ""Использовать ответственное хранение собственными организациями"".';
									|en = 'Sets the ""Use VMI at own companies"" constant value.'");
...
Процедура СоздатьЭлементыСтавкиНДСДляМежценовойРазницы(Параметры) Экспорт 
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстСообщения = НСтр("ru = 'Не удалось удалить дубли в табличной части ""Типы налогообложения НДС"" ставки ""Без НДС"": %Причина%';
								|en = 'Cannot delete duplicates in the ""VAT taxation types"" table of the ""Excluding VAT"" rate: %Причина%'",
				ОбщегоНазначения.КодОсновногоЯзыка());
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
