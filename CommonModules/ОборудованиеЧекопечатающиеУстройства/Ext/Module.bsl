...
Процедура ОбновитьПоставляемыеДрайвера(ДрайвераОборудования) Экспорт
	...
	Драйвер.Наименование = НСтр("ru = 'ШТРИХ-М:ККТ с передачей данных в ОФД (ФФД 1.2) NG';
								|en = 'SHTRIH-M:Cash register with data transfer to FDO (FDF 1.2) NG'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'НЕВА:ККТ с передачей данных в ОФД (ФФД 1.2)';
								|en = 'NEVA:Cash register with data transfer to FDO (FDF 1.2)'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
			...
			Исключение
				...
				ОбщегоНазначенияБПО.ЗаписатьОшибкуВЖурналРегистрации(
					НСтр("ru = 'Ошибка записи в журнал проверки кода маркировки.';
						|en = 'Ошибка записи в журнал проверки кода маркировки.'"),
...
Процедура ВывестиШапкуЧека(Контекст, ОбщиеПараметры, ТипДокумента)
	...
	Если ТипДокумента = 0 Тогда 
		ВывестиСтрокуПоЦентру(Контекст, НСтр("ru = 'КАССОВЫЙ ЧЕК';
											|en = 'SALES SLIP'"), Истина);
	КонецЕсли;
	
	ТипЧека        = ВывестиШапкуТипЧека(ТипДокумента, ОбщиеПараметры.ТипРасчета);
	ОрганизацияИНН = ?(ПустаяСтрока(ОбщиеПараметры.ОрганизацияИНН), "", НСтр("ru = 'ИНН';
	...
	НомерЧека = ?(ПустаяСтрока(ОбщиеПараметры.НомерЧека),"", НСтр("ru = 'ЧЕК №';
	...
	Если ТипДокумента = 0  Тогда
		...
		ВывестиПолеСлева(Контекст, НСтр("ru = 'ПОКУПАТЕЛЬ:';
										|en = 'CUSTOMER:'"),     Покупатель);
		ВывестиПолеСлева(Контекст, НСтр("ru = 'ИНН ПОКУПАТЕЛЯ:';
										|en = 'CUSTOMER TIN:'"), ПокупательИНН);
		ВывестиПолеСлева(Контекст, НСтр("ru = 'АДРЕС ПОКУПАТЕЛЯ:';
										|en = 'CUSTOMER ADDRESS:'"), ПокупательАдрес);
		ВывестиСтрокуСлева(Контекст, ВРег(Строка(ОбщиеПараметры.ПризнакАгента)));
		ВывестиПолеСлева(Контекст, НСтр("ru = 'ПОСТАВЩИК:';
										|en = 'VENDOR:'"), ОбщиеПараметры.ДанныеПоставщика.Наименование);
		ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ТЛФ.ПОСТ.:';
											|en = 'SUPP.PHONE:'"), ОбщиеПараметры.ДанныеПоставщика.Телефон);
		ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ИНН ПОСТАВЩИКА:';
											|en = 'VENDOR TIN:'"), ОбщиеПараметры.ДанныеПоставщика.ИНН);
		ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ПЛ.АГЕНТ ОПЕР.:';
											|en = 'PAYMENT AGENT OPERATION:'"), ОбщиеПараметры.ДанныеАгента.ПлатежныйАгент.Операция);
		ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ПЛ.АГЕНТ.ТЛФ.:';
											|en = 'PAYMENT AGENT PHONE NUMBER:'"), ОбщиеПараметры.ДанныеАгента.ПлатежныйАгент.Телефон);
		ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ОПЕРАТОР ПЕРЕВОДА:';
											|en = 'TRANSACTION OPERATOR:'"), ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Наименование);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'ТЛФ.ОП.ПЕРЕВОДА:';
												|en = 'TRANSACTION OPERATOR PHONE NUMBER:'"), ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Телефон);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'АДР.ОП.ПЕРЕВОДА:';
												|en = 'TRANSACTION OPERATOR ADDRESS:'"), ОбщиеПараметры.ДанныеАгента.ОператорПеревода.Адрес);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'ИНН ОП.ПЕРЕВОДА:';
												|en = 'TRANSACTION OPERATOR TIN:'"), ОбщиеПараметры.ДанныеАгента.ОператорПеревода.ИНН);
		ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ОПЕР.ПО ПРИЕМУ ПЛАТ.ТЛФ:';
											|en = 'OPERATOR TO RECEIVE PAYMENTS PHONE NUMBER:'"), ОбщиеПараметры.ДанныеАгента.ОператорПоПриемуПлатежей.Телефон);
		ВывестиПолеПоШирине(Контекст, НСтр("ru = 'РЕК.ПОЛЬЗ.:';
											|en = 'USER ADVERT:'"), ОбщиеПараметры.ДополнительныйРеквизитПользователя.Наименование);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'РЕК.ПОЛЬЗ.:';
												|en = 'USER ADVERT:'"), ОбщиеПараметры.ДополнительныйРеквизитПользователя.Значение);
	КонецЕсли;
	...
	Если ПозицияЧека.СуммаСкидок<>Неопределено Тогда
		Если ПозицияЧека.СуммаСкидок > 0 Тогда
			ТекстСкидки = НСтр("ru = 'СКИДКА';
								|en = 'DISCOUNT'") + "=" + Формат(ПозицияЧека.СуммаСкидок, Контекст.ФорматЧисло);
		ИначеЕсли ПозицияЧека.СуммаСкидок < 0 Тогда
			ТекстСкидки = НСтр("ru = 'НАДБАВКА';
								|en = 'STANDARD BONUS'") + "=" + Формат(ПозицияЧека.СуммаСкидок, Контекст.ФорматЧисло);
		КонецЕсли;
	КонецЕсли;
...
Процедура ВывестиПозицииЧека(Контекст, ОбщиеПараметры)
	...
	Для Каждого ТекущаяПозицияЧека Из ОбщиеПараметры.ПозицииЧека Цикл
		...
		Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
			...
			ВывестиПолеСлева(Контекст, НСтр("ru = 'КОД СТРАНЫ:';
											|en = 'COUNTRY CODE:'"), ПозицияЧека.КодСтраныПроисхожденияТовара);
			ВывестиПолеСлева(Контекст, НСтр("ru = 'ДЕКЛАРАЦИЯ:';
											|en = 'DECLARATION:'"), ПозицияЧека.НомерТаможеннойДекларации);
			ВывестиПолеСлева(Контекст, НСтр("ru = 'АКЦИЗ:';
											|en = 'EXCISE:'"), Строка(ПозицияЧека.СуммаАкциза));
			ВывестиПолеСлева(Контекст, НСтр("ru = 'ЕД.ИЗМЕРЕНИЯ:';
											|en = 'UNIT OF MEASURE:'"), ПозицияЧека.ЕдиницаИзмерения);
			ВывестиСтрокуСлева(Контекст, ВРег(Строка(ПозицияЧека.ПризнакАгентаПоПредметуРасчета)));
			ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ПОСТАВЩИК:';
												|en = 'VENDOR:'"), ПозицияЧека.ДанныеПоставщика.Наименование);
			ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ТЛФ.ПОСТ.:';
												|en = 'SUPP.PHONE:'"), ПозицияЧека.ДанныеПоставщика.Телефон);
			ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ИНН ПОСТАВЩИКА:';
												|en = 'VENDOR TIN:'"), ПозицияЧека.ДанныеПоставщика.ИНН);
			ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ПЛ.АГЕНТ ОПЕР.:';
												|en = 'PAYMENT AGENT OPERATION:'"), ПозицияЧека.ДанныеАгента.ПлатежныйАгент.Операция);
			// АПК: 1299-выкл особенность форматирования в чеках
			ВывестиПолеПоШирине(Контекст, НСтр("ru = ' ПЛ.АГЕНТ ТЛФ.:';
												|en = ' PAYMENT AGENT PHONE NUMBER:'"), ПозицияЧека.ДанныеАгента.ПлатежныйАгент.Телефон);
			// АПК: 1299-вкл
			ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ОПЕРАТОР ПЕРЕВОДА:';
												|en = 'TRANSACTION OPERATOR:'"), ПозицияЧека.ДанныеАгента.ОператорПеревода.Наименование);
			ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ТЛФ.ОП.ПЕРЕВОДА:';
												|en = 'TRANSACTION OPERATOR PHONE NUMBER:'"), ПозицияЧека.ДанныеАгента.ОператорПеревода.Телефон);
			ВывестиПолеПоШирине(Контекст, НСтр("ru = 'АДР.ОП.ПЕРЕВОДА:';
												|en = 'TRANSACTION OPERATOR ADDRESS:'"), ПозицияЧека.ДанныеАгента.ОператорПеревода.Адрес);
			ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ИНН ОП.ПЕРЕВОДА:';
												|en = 'TRANSACTION OPERATOR TIN:'"), ПозицияЧека.ДанныеАгента.ОператорПеревода.ИНН);
			ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ОПЕР.ПО ПРИЕМУ ПЛАТ.ТЛФ:';
												|en = 'OPERATOR TO RECEIVE PAYMENTS PHONE NUMBER:'"), ПозицияЧека.ДанныеАгента.ОператорПоПриемуПлатежей.Телефон);
			ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ДОП.РЕК.:';
												|en = 'ADD. ATTR.:'"), ПозицияЧека.ДополнительныйРеквизит);
...
Процедура ВывестиТаблицуОплат(Контекст, ОбщиеПараметры)
	...
	ВывестиРазделитель(Контекст, "Сплошная");
	ВывестиРазделитель(Контекст, "НетЛинии");
	ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ИТОГ';
										|en = 'TOTAL'"), Итоги.СуммаЧека);
	ВывестиРазделитель(Контекст, "Сплошная");
	ВывестиРазделитель(Контекст, "НетЛинии");
	...
	Если ФорматФФД = "1.0" Тогда // ФФД 1.0
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'НАЛИЧНЫМИ';
												|en = 'IN CASH'"),    Оплата.СуммаНаличными);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'ЭЛЕКТРОННЫМИ';
												|en = 'ELECTRONIC'"), Оплата.СуммаЭлектронными);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'ЭЛЕКТРОННЫМИ';
												|en = 'ELECTRONIC'"), Оплата.СуммаПостоплатой);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'ЭЛЕКТРОННЫМИ';
												|en = 'ELECTRONIC'"), Оплата.СуммаПредоплатой);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'ЭЛЕКТРОННЫМИ';
												|en = 'ELECTRONIC'"), Оплата.СуммаПредоставлением);
	Иначе // ФФД 1.05 & ФФД 1.1
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'НАЛИЧНЫМИ';
												|en = 'IN CASH'"),       Оплата.СуммаНаличными);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'БЕЗНАЛИЧНЫМИ';
												|en = 'BY BANK CARD'"),    Оплата.СуммаЭлектронными);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'ПРЕДВАРИТЕЛЬНАЯ';
												|en = 'PRELIMINARY'"), Оплата.СуммаПредоплатой);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'ПОСЛЕДУЮЩАЯ';
												|en = 'SUBSEQUENT'"),     Оплата.СуммаПостоплатой);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'ИНАЯ ФОРМА';
												|en = 'OTHER FORM'"),      Оплата.СуммаПредоставлением);
	КонецЕсли; 
	
	Если Оплата.СуммаСдачи > 0 Тогда
		ВывестиПолеПоШирине(Контекст, НСтр("ru = 'ПОЛУЧЕНО';
											|en = 'RECEIVED'"),    Оплата.СуммаОплаты);
		ВывестиПолеПоШирине(Контекст, " " + НСтр("ru = 'СДАЧА';
												|en = 'CHANGE'"), Оплата.СуммаСдачи);
	КонецЕсли; 
	...
	Если ОбщиеПараметры.Свойство("СистемаНалогообложения") Тогда
		...
		ВывестиПолеПоШирине(Контекст, НСтр("ru = 'СНО:';
											|en = 'TS:'"), СистемаНалогообложения);
	КонецЕсли;          
	
	// АПК: 1299-выкл особенность форматирования в чеках
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru = ' СУММА НДС 20%';
												|en = ' VAT AMOUNT 20%'"),     Итоги.СуммаПозицийНДС20,  Итоги.СуммаНДС20, 20/120);
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru = ' СУММА НДС 18%';
												|en = ' VAT AMOUNT 18%'"),     Итоги.СуммаПозицийНДС18,  Итоги.СуммаНДС18, 18/118);
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru = ' СУММА НДС 10%';
												|en = ' VAT AMOUNT 10%'"),     Итоги.СуммаПозицийНДС10,  Итоги.СуммаНДС10, 10/110);
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru = ' СУММА НДС 20/120';
												|en = ' VAT AMOUNT 20/120'"), Итоги.СуммаПозицийНДС120, Итоги.СуммаНДС120, 20/120);
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru = ' СУММА НДС 18/118';
												|en = ' VAT AMOUNT 18/118'"), Итоги.СуммаПозицийНДС118, Итоги.СуммаНДС118, 18/118);
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru = ' СУММА НДС 10/110';
												|en = ' VAT AMOUNT 10/110'"), Итоги.СуммаПозицийНДС110, Итоги.СуммаНДС110, 10/110);
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru = ' СУММА НДС 0%';
												|en = ' VAT AMOUNT 0%'"),      Итоги.СуммаПозицийНДС0);
	ВывестиТаблицуОплатСуммаНДС(Контекст, НСтр("ru = ' СУММА БЕЗ НДС';
												|en = ' AMOUNT EXCLUDING VAT'"),     Итоги.СуммаПозицийБезНДС);
	// АПК: 1299-вкл
	
	ВывестиСтрокуСлева(Контекст,НСтр("ru = 'КАССИР';
									|en = 'CASHIER'") + ":" + Символы.НПП + ОбщиеПараметры.Кассир);
...
Процедура ВывестиРеквизитыЧека(Контекст, ОбщиеПараметры, Реквизиты)
	...
	ВывестиСтрокуПоШирине(Контекст, НСтр("ru = 'РН ККТ:';
										|en = 'Cash register registration number:'"), Реквизиты.РегистрационныйНомерККТ);
	ВывестиСтрокуПоШирине(Контекст, НСтр("ru = 'ФН №:';
										|en = 'Fiscal memory device N.:'"), Реквизиты.ЗаводскойНомерФН);
	ВывестиСтрокуПоШирине(Контекст, НСтр("ru = 'ФД №:';
										|en = 'Fiscal document No:'"), Реквизиты.НомерЧекаККМ);
	ВывестиСтрокуПоШирине(Контекст, НСтр("ru = 'ФП:';
										|en = 'Fiscal flag:'"), Реквизиты.ФискальныйПризнак);
