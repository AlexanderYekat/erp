		...
		Если ТипДокумента = 0  Тогда
			...
			Если НЕ ПустаяСтрока(Покупатель) Тогда
				Текст = Текст + ПостроитьПолеПереносом(НСтр("ru = 'ПОКУПАТЕЛЬ:';
															|en = 'CUSTOMER:'") + Покупатель, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ПокупательИНН) Тогда
				Текст = Текст + ПостроитьПоле(НСтр("ru = 'ИНН ПОКУПАТЕЛЯ:';
													|en = 'CUSTOMER TIN:'") + ПокупательИНН, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ПокупательАдрес) Тогда
				Текст = Текст + ПостроитьПоле(НСтр("ru = 'АДРЕС ПОКУПАТЕЛЯ:';
													|en = 'CUSTOMER ADDRESS:'") + ПокупательАдрес, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			...
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеПоставщика.Наименование) Тогда
				Текст = Текст + ПостроитьПолеПереносом(НСтр("ru = 'ПОСТАВЩИК:';
															|en = 'VENDOR:'") + ОбщиеПараметры.ДанныеПоставщика.Наименование, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеПоставщика.Телефон) Тогда
				Текст = Текст + ВыстроитьПоля(НСтр("ru = 'ТЛФ.ПОСТ.:';
													|en = 'VENDOR PHONE NUMBER:'"), ОбщиеПараметры.ДанныеПоставщика.Телефон, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ОбщиеПараметры.ДанныеПоставщика.ИНН) Тогда
				Текст = Текст + ВыстроитьПоля(НСтр("ru = 'ИНН ПОСТАВЩИКА:';
													|en = 'VENDOR TIN:'"), ОбщиеПараметры.ДанныеПоставщика.ИНН, ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
		...
		Для ИндексМассива = 0 По ПозицииЧека.Количество() - 1 Цикл
			...
			Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				Если ПозицияЧека.Свойство("КонтрольнаяМарка") И НЕ ПустаяСтрока(ПозицияЧека.КонтрольнаяМарка) Тогда 
					Текст = Текст + ПостроитьПоле(НСтр("ru = '[М]';
														|en = '[М]'"), ШиринаСтроки) + Символы.ПС;
				...
				Если НЕ ПустаяСтрока(ПозицияЧека.ЕдиницаИзмерения) Тогда
					Текст = Текст + ПостроитьПоле(НСтр("ru = 'ЕД.ИЗМЕРЕНИЯ:';
														|en = 'UNIT OF MEASURE:'") + ПозицияЧека.ЕдиницаИзмерения, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				...
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеПоставщика.Наименование) Тогда
					Текст = Текст + ВыстроитьПоля(НСтр("ru = 'ПОСТАВЩИК:';
														|en = 'VENDOR:'"), ПозицияЧека.ДанныеПоставщика.Наименование, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеПоставщика.Телефон) Тогда
					Текст = Текст + ВыстроитьПоля(НСтр("ru = 'ТЛФ.ПОСТ.:';
														|en = 'VENDOR PHONE NUMBER:'"), ПозицияЧека.ДанныеПоставщика.Телефон, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПозицияЧека.ДанныеПоставщика.ИНН) Тогда
					Текст = Текст + ВыстроитьПоля(НСтр("ru = 'ИНН ПОСТАВЩИКА:';
														|en = 'VENDOR TIN:'"), ПозицияЧека.ДанныеПоставщика.ИНН, ШиринаСтроки) + Символы.ПС;
				КонецЕсли;
	...
	Если ТаблицаОплат <> Неопределено Тогда
		...
		Если ФорматФФД = "1.0" Тогда // ФФД 1.0
			Если СуммаНаличными > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'НАЛИЧНЫМИ';
														|en = 'IN CASH'"), "=" + Формат(СуммаНаличными, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
		...
		Иначе // ФФД 1.05 & ФФД 1.1
			...
			Если СуммаЭлектронными > 0 Тогда
				Текст = Текст + ВыстроитьПоля(" " + НСтр("ru = 'БЕЗНАЛИЧНЫМИ';
														|en = 'BY BANK CARD'"), "=" + Формат(СуммаЭлектронными, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
			КонецЕсли;
			Если СуммаПредоплатой > 0 Тогда
		...
		Если СуммаПозицийБезНДС > 0 Тогда
			Текст = Текст + ВыстроитьПоля(НСтр("ru = ' СУММА БЕЗ НДС';
												|en = ' AMOUNT EXCLUDING VAT'"), "=" + Формат(СуммаПозицийБезНДС, ФорматЧисла), ШиринаСтроки) + Символы.ПС;
		КонецЕсли;
		// АПК: 1299-вкл
...
Функция СформироватьШаблонИнфоКвитанции(Параметры, ШиринаСтроки, ПечатьШтрихкода = Истина) Экспорт
	...
	ДатаВремя = НСтр("ru = 'ДАТА:';
					|en = 'DATE:'") + Формат(МенеджерОборудованияКлиентСервер.ДатаСеанса(), "ДФ=""дд.ММ.гггг ЧЧ:мм""");
	// АПК: 1367-вкл
