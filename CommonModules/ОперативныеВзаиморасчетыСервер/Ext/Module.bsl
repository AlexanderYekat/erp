...
Процедура ЗаполнитьОперативныеВзаиморасчетыПоТаблице(Параметры, ДополнительныеСвойства = Неопределено) Экспорт
	...
	Для Каждого СтрокаКРаспределению Из ТаблицаКРаспределению Цикл
		СтрокаКРаспределению.ПорядокОперации = Лев(СтрокаКРаспределению.ПорядокОперации, 8) + Сред(СтрокаКРаспределению.ПорядокОперации, 10, 6);
	КонецЦикла;
	ТаблицаКРаспределению.Сортировать("ПорядокОперации, ПоДаннымОбъектаРасчетовИсточника, ПорядокФакт");
...
Процедура ВыполнитьПереоценкуНаДату(МассивОрганизаций, Период, Параметры) Экспорт
	...
	Если РегистрыНУ.ЗаписыватьОднимНабором Тогда
		...
		КонецЕсли;
		Если РегистрыНУ.РасчетПереоценкиНУ.Количество() > 0 Тогда
			Таблица = РегистрыНУ.РасчетПереоценкиНУ.Выгрузить();
			Таблица.Свернуть(
				"Период,Организация,Валюта,Счет,Аналитика1,Аналитика2,Аналитика3,ДокументОплаты,ОбъектРасчетов,ТипСуммы,Получатель,Курс,Кратность,ЭтоТребование",
				"ОстатокВВалюте,ОстатокВРубляхНУ,ОстатокПоКурсуПереоценки,СуммаПереоценкиНУ,ПогашениеВВалюте,ОстатокОД,ОстатокОР,ОтложенныйДоход,ОтложенныйРасход");
			РегистрыНУ.РасчетПереоценкиНУ.Загрузить(Таблица);
			РегистрыНУ.РасчетПереоценкиНУ.Записать(Ложь);
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьОтгрузкиПоставки(Запрос, ГлобальныеПеременные, ЭтоРасчетыСКлиентами = Ложь)
	...
	|
	|	ВЫБОР
	|		КОГДА ВтПараметрыРасчета.ПараметрГрафикВДоговоре И НЕ Расчеты.Регистратор ССЫЛКА Документ.ВзаимозачетЗадолженности ТОГДА ВтПараметрыРасчета.ПараметрГрафикИсполненияДоговора
	...
	|
	|	ВЫБОР
	|		КОГДА ВтПараметрыРасчета.ПараметрГрафикВДоговоре И НЕ Расчеты.Регистратор ССЫЛКА Документ.ВзаимозачетЗадолженности 
...
Процедура ЗаполнитьДвиженияНУзаПериод(Организация, Период, ЭтоРасчетыСКлиентами, ДатаНачалаПересчета = Неопределено)
	...
	|			ИЛИ ТИПЗНАЧЕНИЯ(Расчеты.Регистратор) = ТИП(Документ.ВводОстатков))
	|		И Расчеты.Валюта <> Аналитика.ВалютаРегламентированногоУчета
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	...
	|	Расчеты.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|	Расчеты.ОбъектРасчетов КАК ОбъектРасчетов,
	|	Расчеты.ОбъектРасчетов.Объект КАК ОбъектРасчетовОбъект,
	|	Расчеты.ОбъектРасчетов.ТипРасчетов КАК ТипРасчетов,
	|	Аналитика.Договор КАК Договор,
	|	Аналитика.ВариантКурсаДоговора КАК ВариантКурсаДоговора,
	|	Аналитика.ВалютаРегламентированногоУчета КАК ВалютаРегл,
	|	Расчеты.Валюта КАК Валюта,
	|	Расчеты.РасчетныйДокумент КАК РасчетныйДокумент,
	|	Расчеты.Долг КАК Долг,
	|	Расчеты.ДолгРегл КАК ДолгРегл,
	|	&ЭтоРасчетыСКлиентами КАК ЭтоРасчетыСКлиентами
	|ПОМЕСТИТЬ втДвиженияРасчетов
	|ИЗ
	|	РегистрНакопления.РасчетыСКлиентамиПоСрокам КАК Расчеты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтАналитикиУчетаПоПартнерам КАК Аналитика
	|		ПО Расчеты.АналитикаУчетаПоПартнерам = Аналитика.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ втВводОстатковНУ КАК ВводОстатковНУ
	|		ПО Расчеты.ДокументРегистратор = ВводОстатковНУ.Ссылка
	|			И Расчеты.АналитикаУчетаПоПартнерам = ВводОстатковНУ.АналитикаУчетаПоПартнерам
	|			И Расчеты.ОбъектРасчетов = ВводОстатковНУ.ОбъектРасчетов
	|			И Расчеты.Валюта = ВводОстатковНУ.Валюта
	|ГДЕ
	|	Расчеты.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И Расчеты.Валюта <> Аналитика.ВалютаРегламентированногоУчета
	|	И ТИПЗНАЧЕНИЯ(Расчеты.ДокументРегистратор) <> ТИП(Документ.РасчетКурсовыхРазниц)
	|	И ВводОстатковНУ.Ссылка ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДвиженияРасчетов.Организация КАК Организация,
	|	ДвиженияРасчетов.Валюта КАК ВалютаВзаиморасчетов,
	|	ДвиженияРасчетов.Период КАК Дата
	|ПОМЕСТИТЬ ТаблицаПериодов
	|ИЗ
	|	втДвиженияРасчетов КАК ДвиженияРасчетов
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	...
	|		И КурсыВалют.БазоваяВалюта = Организации.ВалютаРегламентированногоУчета
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаПериодов.ВалютаВзаиморасчетов,
	|	ТаблицаПериодов.Организация,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаПериодов.ВалютаВзаиморасчетов КАК Валюта,
	...
	|ПОМЕСТИТЬ втКурсыВалют
	|ИЗ
	|	ТаблицаПериодовВзаиморасчетов КАК ТаблицаПериодов
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтносительныеКурсыВалют КАК КурсыВалют
	|	ПО
	|		КурсыВалют.Период = ТаблицаПериодов.ДатаКурса
	|		И КурсыВалют.Валюта = ТаблицаПериодов.ВалютаВзаиморасчетов
	|		И КурсыВалют.БазоваяВалюта = ТаблицаПериодов.ВалютаРегл
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДвиженияРасчетов.Договор КАК Договор,
	|	ДвиженияРасчетов.Валюта КАК ВалютаВзаиморасчетов,
	|	ДвиженияРасчетов.Период КАК Дата
	|ПОМЕСТИТЬ втПериодыПоДоговорам
	|ИЗ
	|	втДвиженияРасчетов КАК ДвиженияРасчетов
	|ГДЕ
	|	ДвиженияРасчетов.ВариантКурсаДоговора = ЗНАЧЕНИЕ(Перечисление.ВариантыКурсаДоговора.УстановленныйВДоговоре)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Договор,
	|	ВалютаВзаиморасчетов,
	...
	|	ТаблицаПериодов.ВалютаВзаиморасчетов,
	|	ТаблицаПериодов.Дата,
	|	ТаблицаПериодов.Договор,
	|	МАКСИМУМ(КурсыВалют.Период) КАК ДатаКурса
	|ПОМЕСТИТЬ ТаблицаПериодовДоговоров
	|ИЗ
	|	втПериодыПоДоговорам КАК ТаблицаПериодов
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.КурсыВалютРасчетовПоДоговорам КАК КурсыВалют
	|	ПО
	|		КурсыВалют.Договор = ТаблицаПериодов.Договор
	|		И КурсыВалют.Период <= ТаблицаПериодов.Дата
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаПериодов.ВалютаВзаиморасчетов,
	...
	|	ТаблицаПериодов.ВалютаВзаиморасчетов КАК Валюта,
	|	ТаблицаПериодов.Дата КАК Дата,
	|	ТаблицаПериодов.Договор КАК Договор,
	|	КурсыВалют.КурсЧислитель КАК КурсЧислитель,
	|	КурсыВалют.КурсЗнаменатель КАК КурсЗнаменатель
	|ПОМЕСТИТЬ втКурсыВалютПоДоговорам
	|ИЗ
	|	ТаблицаПериодовДоговоров КАК ТаблицаПериодов
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалютРасчетовПоДоговорам КАК КурсыВалют
	|	ПО
	|		КурсыВалют.Период = ТаблицаПериодов.ДатаКурса
	|		И КурсыВалют.Договор = ТаблицаПериодов.Договор
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Расчеты.АналитикаУчетаПоПартнерам КАК АналитикаУчетаПоПартнерам,
	|	Расчеты.ОбъектРасчетов КАК ОбъектРасчетов,
	|	Расчеты.ОбъектРасчетовОбъект КАК ОбъектРасчетовОбъект,
	|	Расчеты.ТипРасчетов КАК ТипРасчетов,
	|	Расчеты.Договор КАК Договор,
	|	Расчеты.ВариантКурсаДоговора КАК ВариантКурсаДоговора,
	|	Расчеты.ВалютаРегл КАК ВалютаРегл,
	|	Расчеты.Валюта КАК Валюта,
	|	Расчеты.РасчетныйДокумент КАК РасчетныйДокумент,
	|	Расчеты.Долг КАК Долг,
	|	Расчеты.ДолгРегл КАК ДолгРегл,
	|	ВЫБОР КОГДА Расчеты.ВариантКурсаДоговора = ЗНАЧЕНИЕ(Перечисление.ВариантыКурсаДоговора.УстановленныйВДоговоре)
	|			ТОГДА ЕСТЬNULL(КурсыВалютПоДоговорам.КурсЧислитель,1)
	|		КОГДА Расчеты.ВариантКурсаДоговора = ЗНАЧЕНИЕ(Перечисление.ВариантыКурсаДоговора.ФиксированныйНаДатуОтгрузки)
	|				И Расчеты.ТипРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовРасчетов.Накладная)
	|			ТОГДА ЕСТЬNULL(КурсыВалютПоДокументам.КурсЧислительВалютыВзаиморасчетов,1)
	|		ИНАЧЕ 
	|			ЕСТЬNULL(КурсыВалют.КурсЧислитель,1)
	|	КОНЕЦ                                                                           КАК КурсЧислительРегл,
	...
	|			ЕСТЬNULL(КурсыВалют.КурсЗнаменатель,1)
	|	КОНЕЦ                                                                           КАК КурсЗнаменательРегл,
	|	Расчеты.ЭтоРасчетыСКлиентами КАК ЭтоРасчетыСКлиентами
	|ПОМЕСТИТЬ втДвиженияРасчетовСКурсамиВалют
	|ИЗ
	|	втДвиженияРасчетов КАК Расчеты
	|	ЛЕВОЕ СОЕДИНЕНИЕ втКурсыВалют КАК КурсыВалют
	|		ПО Расчеты.Период = КурсыВалют.Дата
	|		И Расчеты.Валюта = КурсыВалют.Валюта
	|		И Расчеты.ВалютаРегл = КурсыВалют.БазоваяВалюта
	|	ЛЕВОЕ СОЕДИНЕНИЕ втКурсыВалютПоДоговорам КАК КурсыВалютПоДоговорам
	|		ПО Расчеты.Период = КурсыВалютПоДоговорам.Дата
	|		И Расчеты.Договор = КурсыВалютПоДоговорам.Договор
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВалютыИКурсыДокументов КАК КурсыВалютПоДокументам
	|		ПО Расчеты.ОбъектРасчетовОбъект = КурсыВалютПоДокументам.Документ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		Расчеты.РасчетныйДокумент КАК РасчетныйДокумент,
	|		Расчеты.Долг КАК Долг,
	|		ВЫРАЗИТЬ(Расчеты.Долг
	|			* Расчеты.КурсЧислительРегл / Расчеты.КурсЗнаменательРегл КАК ЧИСЛО(31,2)) КАК ДолгРегл,
	|		Расчеты.ЭтоРасчетыСКлиентами КАК ЭтоРасчетыСКлиентами
	|	ИЗ
	|		втДвиженияРасчетовСКурсамиВалют КАК Расчеты
	|) КАК Обороты
	|
	|СГРУППИРОВАТЬ ПО
	|	Обороты.Период,
...
Функция РассчитатьСуммыНУ(Период, КурсНаДату, ЗаписьНУ, Оплата = Неопределено)
	...
	НачальныйОстатокДолгаНУ = ЗаписьНУ.СуммаНУОстаток + Окр(Оплата.ДолгРасход * КурсНаДату, 2);//в регистр еще не записана курсовая разница
	...
	Если НачальныйОстатокДолга <> 0 Тогда
		...
		Если Оплата.ДолгРасход <> 0 Тогда
			КурсоваяРазницаОплаты = Окр(-Оплата.ДолгРасход * (КурсОстаткаНУ - КурсНаДату), 2);
		КонецЕсли;
