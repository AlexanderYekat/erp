...
Процедура ПроверитьЗаполнениеПериодовВыплатыПособий(Объект, Отказ, ПроверяемыеРеквизиты, Знач ДатаНачалаОтпуска = Неопределено,
	...
	Если Объект.ВыплачиватьПособиеДоПолутораЛет Тогда
		...
		ИначеЕсли Объект.ДатаОкончанияПособияДоПолутораЛет < ДатаНачалаОтпуска Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,"Объект.ДатаОкончанияПособияДоПолутораЛет",, Отказ);
		ИначеЕсли Объект.ДатаОкончанияПособияДоПолутораЛет < НачалоДня(Объект.Дата) Тогда
			ТекстСообщения = НСтр("ru = 'Дата окончания выплаты пособия за счет ФСС до полутора лет не должна быть меньше даты документа.';
									|en = 'The end date of child care allowance for children under one and a half years old paid by SSF cannot be earlier than the document date.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,,"Объект.ДатаОкончанияПособияДоПолутораЛет",, Отказ);
		КонецЕсли;
	КонецЕсли;
