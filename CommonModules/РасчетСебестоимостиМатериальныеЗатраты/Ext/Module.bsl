...
Функция ТекстРасчетноеРаспределениеНоменклатурыНаВыпуск()
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|
		// корректировка плана материалов этапов на экономию материала (логика идентична отмененным строкам в обеспечении)
	|	ВЫБРАТЬ
	|		ДД.Организация                КАК Организация,
	|		ДД.ПартияПроизводства         КАК ПартияПроизводства,
	|		Обеспечение.Номенклатура      КАК Номенклатура,
	...
	|		КОНЕЦ                         КАК Назначение,
	|		Обеспечение.СтатьяКалькуляции КАК СтатьяКалькуляции,
	|		-ЭкономияМатериалов.Количество КАК План,
	|		0                             КАК Факт,
	|		1                             КАК КоэффициентПриведения
	|	ИЗ
	|		ПартииПроизводстваСводно КАК ДД
	...
	|			ПО ДД.ПартияПроизводства = ЭкономияМатериалов.Ссылка.ПартияПроизводства
	|			И Обеспечение.КорАналитикаУчетаНоменклатуры = ЭкономияМатериалов.АналитикаУчетаНоменклатуры
	|
	|	ГДЕ
	|		НЕ Обеспечение.Ссылка.ПроизводствоНаСтороне
	|		И НЕ Обеспечение.Отменено
