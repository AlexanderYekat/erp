...
Функция ТекстСвязиУзловРезервовПодОбесценениеЗапасов()
	...
	Возврат "
		...
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаНоменклатуры КАК СпрАналитикаУчетаНоменклатуры
		|		ПО (СпрАналитикаУчетаНоменклатуры.Ссылка = Себестоимость.АналитикаУчетаНоменклатуры)
		|
...
Функция ТекстВтТаблицаСвязейНУ() // вт ВтТаблицаСвязейНУ 
	Возврат "
		...
		|ИЗ
		|	ВтТаблицаСвязейПостатейныеРасходы КАК ПеремещенияСписания
		|	ЛЕВОЕ СОЕДИНЕНИЕ УзлыКорректировкиДополнительныхРасходов КАК УзлыКорректировкиДопРасходовИсточник
		|		ПО УзлыКорректировкиДопРасходовИсточник.НомерУзла = ПеремещенияСписания.НомерУзлаИсточник
		|ГДЕ
		|	НЕ ПеремещенияСписания.НомерУзлаПриемник ЕСТЬ NULL
		|	И НЕ ПеремещенияСписания.НомерУзлаИсточник ЕСТЬ NULL
		// Начальная стоимость производственных затрат при списании на них доп. расходов уже сформирована,
		// поэтому необходимо отключить дугу из узла доп. расходов.
		|	И УзлыКорректировкиДопРасходовИсточник.НомерУзла ЕСТЬ NULL
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|	
		|ВЫБРАТЬ
...
Процедура РешитьСЛУ_НалоговыйУчет_ПостояннаяРазница(ПараметрыРасчета)
	...
	|	УзлыКорректировки.НомерУзла КАК НомерУзла,
	|
	// Начальная стоимость ПР узлов себестоимости увеличивается на суммы НУ, которые рассчитаны в РешитьСЛУПлатформой_НалоговыйУчет,
	// при списании ТМЦ/работ на непринимаемые в НУ постатейные расходы
	|	ВЫРАЗИТЬ(СУММА(ВЫБОР
	|		КОГДА ЕСТЬNULL(НЕ ПеремещенияСписания.ПринимаемыеВНУ, ЛОЖЬ)
	...
	РасчетСебестоимостиПротоколРасчета.НачалоЭтапаРасчета(ПараметрыРасчета, "РешитьСЛУПлатформой_ПостоянныеРазницы_Этап2");
	
	ПояснениеДляЗамера = НСтр("ru = 'Решение для постоянных разниц (этап 2)';
								|en = 'Solution for permanent differences (stage 2)'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ТекстСтоимостьПартийТоваров()
	Возврат "
		...
		|
		//++ Локализация
		|	ВЫРАЗИТЬ(ЕСТЬNULL(СебестоимостьНалоговыйУчет.СуммаНУ, 0) КАК ЧИСЛО(28,10))                   КАК СтоимостьНУ,
