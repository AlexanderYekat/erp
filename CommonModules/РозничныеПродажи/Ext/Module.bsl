		...
		Если Не ТолькоЗаполнить Тогда
			...
			Иначе
				ОписаниеОшибки = НСтр("ru = 'При проверке заполнения перед проведением документа %Документ% произошла ошибка. Документ записан, но не проведен.';
										|en = 'An error occurred when posting the %Документ% document. The document is saved but not posted.'");
...
Функция ЗаполнитьОтчетОРозничныхВозвратах(ОтчетОРозничныхВозвратахОбъект, СостояниеКассовойСмены, НалогообложениеНДС, МенеджерВременныхТаблиц, ТолькоЗаполнить = Ложь, ОписаниеОшибки = "") Экспорт
	...
	Исключение
		...
		ОписаниеОшибки = НСтр("ru = 'При создании отчета о розничных возвратах произошла ошибка.
		                            |Дополнительное описание:
		                            |%ДополнительноеОписание%';
		                            |en = 'An error occurred when generating the retail return report.
		                            |Additional details:
		                            |%ДополнительноеОписание%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", ИнформацияОбОшибке().Описание);
