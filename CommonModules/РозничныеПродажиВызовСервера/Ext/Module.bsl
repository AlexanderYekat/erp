	...
	Если Не ДокументОбъект = Неопределено Тогда
		Если (ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЧекККМ") Или
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Пробитый чек не может быть удален';
																	|en = 'An issued receipt cannot be deleted'"));
			Результат = Ложь;
		...
		Иначе
			...
			Исключение
				Результат = Ложь;
				ШаблонТекстаОшибки = НСтр("ru = 'При отмене фискальной операции не удалось удалить документ %1 %2 от %3 по причине: %4';
											|en = 'Cannot delete the %1 %2 document dated %3 when cancelling the fiscal transaction. Reason: %4'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()); 
