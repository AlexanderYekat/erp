...
Функция КритерииОтбораПоРегистрацииРеспондента(ПараметрыОтчета)
	...
	Если СвойстваРеспондента.ЭтоОрганизация Тогда
		...
		Запрос = Новый Запрос(
			...
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТерриторииСотрудников КАК ТерриторииСотрудников
		...
		ИсключаемыеРегистрации = РезультатЗапроса[0].Выгрузить().ВыгрузитьКолонку("Регистрация");
		...
		Если Не РезультатЗапроса[1].Пустой() Тогда
			ИсключаемыеПоТерриториям = РезультатЗапроса[1].Выгрузить();
	...
	Иначе
		...
		Запрос.УстановитьПараметр("Регистрация", ПараметрыОтчета.РегистрацияРеспондента);
		Запрос.УстановитьПараметр("ДатаАктуальности", ПараметрыОтчета.ДатаНачалаПериодаОтчета);
		...
		Если Не РезультатЗапроса[0].Пустой() Тогда
			ВключаемыеПоТерриториям = РезультатЗапроса[0].Выгрузить();
