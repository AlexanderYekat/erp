...
Процедура РассчитатьСтавкиПроцентыДоходнойАренды(ПараметрыРасчета, Отказ = Ложь) Экспорт
	...
	Запрос.Текст = 
		...
		|		КОНЕЦ - втТаблицаОС.НГЛСРегл) * &КоэффициентПересчетаИзВалютыРегл КАК ЧИСЛО(31, 2)) КАК ПриведеннаяСтоимость,
		|	ВЫБОР
		|		КОГДА втПроцентыФактические.ОсновноеСредство ЕСТЬ NULL
		|			ТОГДА 0
		|		ИНАЧЕ втТаблицаОС.АрендныеПлатежи - ЕСТЬNULL(втПроцентыФактические.ПроцентыФакт, 0) - ЕСТЬNULL(ПроцентыДоИзменения.ПроцентныйДисконт, 0)
		|	КОНЕЦ КАК ПриведеннаяСтоимостьФакт,
		|	втТаблицаОС.АрендныеПлатежи КАК АрендныеПлатежи,
		...
		|	ЕСТЬNULL(АрендныеПлатежиДоИзменения.АрендныеПлатежи, 0) / &КоэффициентПересчетаИзВалютыРегл - ЕСТЬNULL(ПроцентыДоИзменения.ПроцентныйДисконт, 0) / &КоэффициентПересчетаИзВалютыРегл КАК ОстатокЧИА,
	...
	Для Каждого ДанныеСтроки Из ТаблицаОС Цикл
		...
		Если ЭтоИзменениеУсловий Тогда
			...
			Расшифровка.Вставить("ИтогоЧИА", ДанныеСтроки.ОстатокЧИА + ДанныеСтроки.СуммаРасходов - ДанныеСтроки.ИзменениеНГЛС);
		Иначе
			...
			Расшифровка.Вставить("СуммаРасходов", ДанныеСтроки.СуммаРасходов);
			Расшифровка.Вставить("ИтогоИнвестиция", ДанныеСтроки.ИтогоИнвестиция);
			Расшифровка.Вставить("НГЛСРегл", ДанныеСтроки.НГЛСРегл);
			Если ДанныеСтроки.СправедливаяСтоимость > 0 Тогда
				Расшифровка.Вставить("ИтогоЧИА", ДанныеСтроки.СправедливаяСтоимость - ДанныеСтроки.НГЛСРегл);
			Иначе
				Расшифровка.Вставить("ИтогоЧИА", ДанныеСтроки.ИтогоИнвестиция - ДанныеСтроки.НГЛСРегл);
			КонецЕсли;
	...
	Для Каждого ДанныеСтроки Из ТаблицаОС Цикл
		...
		ДанныеСтроки.РасшифровкаРасчета.Вставить("АрендныеПлатежи", АрендныеПлатежи);
		ДанныеСтроки.РасшифровкаРасчета.Вставить("ПриведеннаяСтоимость", ДанныеСтроки.ПриведеннаяСтоимость);
		...
		ДанныеСтроки.РасшифровкаРасчета.Вставить("ПриведеннаяСтоимостьРегл",
			ДанныеСтроки.РасшифровкаРасчета.ИтогоЧИА - ДанныеСтроки.АвансовыеПлатежиРегл);
		ДанныеСтроки.РасшифровкаРасчета.Вставить("ОстатокПроцентов", ДанныеСтроки.ОстатокПроцентов);
		ДанныеСтроки.РасшифровкаРасчета.Вставить("ИзменениеПроцентов", ОстатокПроцентов - ДанныеСтроки.ОстатокПроцентов);
...
Процедура ЗаполнитьСтавкуДоходностиГрафикПроцентов(Форма, ПараметрыРасчета, ПересчитатьПроценты) Экспорт
	...
	Для Каждого СтрокаОС Из ПараметрыРасчета.ТаблицаОС Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			ЗаполнитьЗначенияСвойств(НайденнаяСтрока, СтрокаОС, "СтавкаДоходности, ПриведеннаяСтоимость, РасшифровкаРасчета");
		КонецЦикла;
	КонецЦикла;
