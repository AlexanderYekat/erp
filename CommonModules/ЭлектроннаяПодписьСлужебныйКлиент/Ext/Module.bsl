		...
		Если Не Результат.УстановкаВыполнена Тогда
			...
			ТекстОшибкиУстановкиСпискаОтзыва = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось установить список отзыва при проверке сертификата: %1';
					|en = 'Cannot install the revocation list when checking the certificate: %1'"), Результат.Сообщение);
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для получения данных о программе для работы с сертификатом электронной подписи требуется установка внешней компоненты %1.';
				|en = 'To get data about the application for digital signature certificates, install the %1 add-in.'"),
			"ExtraCryptoAPI");
...
Процедура ПродолжитьПослеОперацииУсовершенствованияНаСервере(РезультатВыполнения, Контекст) Экспорт
	...
	Если РезультатВыполнения = Неопределено Тогда
		...
		ОшибкаНаСервере.Текст = НСтр("ru = 'Усовершенствование подписи фоновым заданием не было завершено штатно';
									|en = 'Усовершенствование подписи фоновым заданием не было завершено штатно'");
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(Результат) Тогда
			ТекстОшибки = НСтр("ru = 'Фоновое задание не вернуло результат';
								|en = 'Фоновое задание не вернуло результат'");
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	Исключение
		ОшибкаНаСервере = ОписаниеОшибкиПриУсовершенствовании(Ложь);
...
Процедура ПродолжитьУстановкуСпискаОтзываПослеСкачивания(РезультатВыполнения, Контекст) Экспорт
	...
	Если РезультатВыполнения = Неопределено Тогда
		ОбработатьОшибкуУстановкиСпискаОтзыва(НСтр("ru = 'Загрузка списка отзыва не была завершена штатно';
													|en = 'Загрузка списка отзыва не была завершена штатно'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для получения данных об установленных программах для работы с электронной подписью требуется установка внешней компоненты %1.';
				|en = 'To get data about the installed applications for digital signatures, install the %1 add-in.'"),
			"ExtraCryptoAPI");
...
Процедура УстановитьКриптоПро(Форма, ОповещениеОЗавершении, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку( 
			...
			|en = 'You can automatically install %1 only on Windows operating systems. 
			|You can download the application distribution package from %2 and install it on your own.'"),
