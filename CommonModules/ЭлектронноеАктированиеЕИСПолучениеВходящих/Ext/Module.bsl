...
Процедура ПолучитьНовыеДокументыЕИС(
	...
	ИмяСобытия = НСтр("ru = 'Получение транспортных контейнеров из ЕИС';
						|en = 'Получение транспортных контейнеров из ЕИС'",
		КодЯзыка);
	Попытка
	...
	Исключение
		ЗаголовокПроблемы = НСтр("ru = 'Ошибка при получении документов поставщика';
								|en = 'Ошибка при получении документов поставщика'");
		...
		ПодробноОбОшибке = НСтр("ru = 'Произошла критическая ошибка при получении документов поставщика';
								|en = 'Произошла критическая ошибка при получении документов поставщика'");
	...
	Исключение
		ЗаголовокПроблемы = НСтр("ru = 'Ошибка при получении документов заказчика';
								|en = 'Ошибка при получении документов заказчика'");
		ОписаниеОшибки = ИнформацияОбОшибке();
		КраткоеОписание = ОбработкаОшибок.КраткоеПредставлениеОшибки(ОписаниеОшибки);
		...
		ПодробноОбОшибке = НСтр("ru = 'Произошла критическая ошибка при получении документов заказчика';
								|en = 'Произошла критическая ошибка при получении документов заказчика'");
	...
	Пока НЕ ПолученыВсеДокументы Цикл
		...
		Если НЕ РезультатПолученияСпискаДокументов.Выполнено Тогда
			...
			ИмяСобытия = НСтр("ru = 'Ошибка при получении документов заказчика из ЕИС в сфере закупок';
								|en = 'An error occurred when receiving the customer data from UIS in procurement'",
				КодОсновногоЯзыка);
		...
		Для Каждого ОписаниеДокумента Из МассивОписанийДокументов Цикл
			...
			Если ДвоичныеДанныеДокумента = Неопределено Тогда
				ИмяСобытия = НСтр("ru = 'Отсутствуют данные документа ЕИС.';
									|en = 'UIS document data is missing.'");
				...
				Если ЗначениеЗаполнено(КонтейнерТитулаПоставщика) Тогда
					...
					Если НЕ РезультатИзмененияСтатуса Тогда
						...
						ИмяСобытия = НСтр("ru = 'Не удалось изменить статус транспортного контейнера.';
											|en = 'Cannot change the transport container status.'");
						ВызватьИсключение(ИмяСобытия);
					КонецЕсли;
			...
			Исключение
				...
				ИмяСобытия = НСтр("ru = 'Сохранение транспортного контейнера документа ЕИС в сфере закупок';
									|en = 'Save the UIS document transport container in procurement'",
					КодЯзыка);
...
Функция ВидОшибкиСбойПриПолученииДокументов(Идентификатор, ЗаголовокПроблемы, ТекстОшибки)
	...
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Произошла ошибка:';
										|en = 'Произошла ошибка:'") + " " + ТекстОшибки;
	ВидОшибки.ОписаниеРешения = НСтр("ru = 'См. подробности ошибки в журнале регистрации';
									|en = 'См. подробности ошибки в журнале регистрации'");
