...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Объект.Этап.Пустая() Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			ЗаполнитьЗначенияСвойств(Объект, Выборка, "Операция, ИдентификаторОперации, ЭтоПерваяОперация");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Индекс = 0 По Объект.Партии.Количество()-1 Цикл
		...
		Если Строка.ДоляСтоимости = 0
			...
			И Не Объект.СНачалаПартии Тогда
...
Процедура НастроитьПартии()
	...
	// Количество партий
	Если Объект.Действие = Действия().Брак Тогда
		КоличествоПартий = ?(Объект.КоличествоУпаковокБрак < Объект.КоличествоУпаковок, 2, 1);
...
Процедура НастроитьПростойРежим()
	...
	ИначеЕсли Объект.Действие = Действия.Брак Тогда
		...
		ПерваяПартия.ДоляСтоимости = Объект.КоличествоУпаковокБрак;
		...
		Если Объект.Партии.Количество() = 2 Тогда
			...
			ВтораяПартия.ДоляСтоимости = Объект.КоличествоУпаковок - Объект.КоличествоУпаковокБрак;
