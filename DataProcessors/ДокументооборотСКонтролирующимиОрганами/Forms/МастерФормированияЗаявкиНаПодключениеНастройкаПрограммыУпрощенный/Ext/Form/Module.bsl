	...
	Если АдресРасписки = "" Тогда
		...
		Если АдресРасписки = "" Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не удалось скачать сертификат с сервера удостоверяющего центра. Попробуйте позже.';
															|en = 'Cannot download the certificate from certification authority server. Try again later.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция СертификатДействителен()
	...
	Если СертификатПросрочен
		...
		ТекстОшибки = НСтр("ru = 'Сертификат, которым выполняется подписание, просрочен';
							|en = 'The certificate being used to sign has expired'");
...
Процедура ИзменитьОформлениеСтраницыУспешноеЗавершение()
	...
	Если ЭтоПервичноеЗаявление Тогда
		...
		Если Мультирежим.ЭтоМультиРежимПоОрганизации(Организация) Тогда
			Шаблон = НСтр("ru = 'Поздравляем!
		                   ...
		                   |en = 'Congratulations!
		                   |You are successfully connected to 1C Reporting'");
		Иначе
			Шаблон = НСтр("ru = 'Поздравляем!
		                   |Организация %1 успешно подключена к 1С-Отчетности';
		                   |en = 'Congratulations!
		                   |Company %1 has been connected to 1C Reporting successfully'");
	...
	Если ПодключитьЭДО Тогда
		Элементы.ЗавершениеНастройки1СЭДО.Заголовок = НСтр("ru = 'Далее будет выполнено завершение подключения к сервису 1С-ЭДО.';
															|en = 'Connection to 1C:EDI service will be completed next.'");
	...
	ИначеЕсли ПереиздатьСертификатЭДО Тогда
		Элементы.ЗавершениеНастройки1СЭДО.Заголовок = НСтр("ru = 'Далее будет выполнено завершение обновления сертификата сервиса 1С-ЭДО.';
															|en = '1C-EDI service certificate will be updated next.'");
...
Функция ИмяКнопкиПереизданияСертификатаЭДО()
	...
	Возврат НСтр("ru = 'Завершить обновление сертификата 1С-ЭДО';
				|en = 'Complete 1C-EDI certificate update'");
		...
		Если Ответ = Неопределено Тогда
			...
			Результат.ОписаниеОшибки = 
				НСтр("ru = 'Не удалось отправить расписку по причине: ';
					|en = 'Cannot send the notice of receipt due to:'") 
				+ Символы.ПС 
				+ Результат.ОписаниеОшибки;
	...
	Исключение
		...
		ДокументооборотСКО.ОбработатьИсключение(
			...
			НСтр("ru = 'Отправка расписки в УЦ Основание (метод SendCertificateForm)';
				|en = 'Send the notice of receipt to Osnovanie CA (the SendCertificateForm method)'"));
		...
		Если Ответ = Неопределено Тогда
			...
			Результат.ОписаниеОшибки = 
				НСтр("ru = 'Не удалось получить срок отправки расписки по причине: ';
					|en = 'Cannot get the period to send the notice of receipt due to:'") 
				+ Символы.ПС 
				+ Результат.ОписаниеОшибки;
	...
	Исключение
		...
		ДокументооборотСКО.ОбработатьИсключение(
			...
			НСтр("ru = 'Получение срок отправки расписки в УЦ Основание (метод GetReceiptRemainingTime)';
				|en = 'Get the period to send the notice of receipt to Osnovanie CA (the GetReceiptRemainingTime method)'"));
...
Процедура ОткрытьНаСтраницеПросроченаОтправкаРасписки()
	...
	АктивизироватьСтраницу(
		...
		НСтр("ru = 'Обратитесь в ФНС для получения сертификата';
			|en = 'Contact FTS to get a certificate'"));
...
Процедура ИзменитьОформлениеГруппыПустаяРаспискаДЛ()
	...
	Элементы.ОшибкаПолученияРаспискиДЛ.Заголовок = 
		НСтр("ru = 'Сервер АО ""Калуга Астрал"" вернул пустую расписку';
			|en = 'The Kaluga Astral JSC server returned an empty notice of receipt'");
...
Процедура ИзменитьОформлениеГруппыОтправитьРаспискуДЛ()
	...
	Если ЗначениеЗаполнено(КрайнийСрокДЛ) Тогда
		...
		Текст = Новый ФорматированнаяСтрока(
			...
			ДокументооборотСКОКлиентСервер.Жирным(НСтр("ru = 'В противном случае новый и текущий сертификаты будут отозваны УЦ АО ""Аналитический центр"".';
														|en = 'Otherwise, the new and current certificates will be revoked by CA JSC Analytical Center.'")));
	...
	Если ТребуетсяПолучениеРасписки Тогда
		Элементы.НастроитьАвтоматически.Заголовок = НСтр("ru = 'Подписать и отправить расписку';
														|en = 'Sign and send the notice of receipt'");
	КонецЕсли;
