...
Функция ЗаголовокЭлементаОтсылкаКЗаявлению()
	...
	Если ИзначальныйВидЗаявленияПФР = НаПодключение Тогда
		Часть4 = НСтр("ru = ', в котором подключили направление СФР.';
						|en = ', where you enabled the Social Insurance Fund of Russia as a recipient.'");
	...
	Если ПоказыватьБыстроеДействие Тогда
		...
		Если ИзначальныйВидЗаявленияПФР = НаПодключение Тогда
			...
			Рекомендация = НСтр("ru = 'Вам также необходимо отправить в СФР заявление на подключение к эл. документообороту и дождаться положительного результата его рассмотрения.';
								|en = 'You also need to send an application to the Social Insurance Fund of Russia to connect to EDI and wait for a positive result of its review.'");
		КонецЕсли;
