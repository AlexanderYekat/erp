...
Процедура ИзменитьОформлениеСтраницыОжидаетсяРезультатРассмотрения(Форма)
	...
	Если Вид = Форма.НаПодключение Тогда
		ПредставлениеВидаЗаявления = НСтр("ru = ' на подключение обмена эл. документами с СФР ';
											|en = ' to enable electronic document exchange with Social Insurance Fund of Russia '");
	ИначеЕсли Вид = Форма.НаОтключение Тогда
		ПредставлениеВидаЗаявления = НСтр("ru = ' на отключение обмена эл. документами с СФР ';
											|en = ' to disable electronic document exchange with Social Insurance Fund of Russia '");
	КонецЕсли;
	...
	Если Форма.ЭтоПредупреждениеИзНастроек Тогда
		...
		СсылкаНаЗаявление = Новый ФорматированнаяСтрока(НСтр("ru = 'заявление';
															|en = 'application'"),,,,НСтр("ru = 'заявление';
																						|en = 'application'"));
		Подстрока2 = НСтр("ru = 'еще не обработано.';
							|en = 'not processed yet.'");
	...
	Если Форма.ЭтоПредупреждениеИзОтправки Тогда
		...
		Шаблон    = НСтр("ru = 'Отправляемый %1 может быть не принят, поскольку заявление %2 еще не обработано СФР.';
						|en = 'The %1 being sent may be not accepted because the %2 application is not processed yet by the Social Insurance Fund of Russia.'");
