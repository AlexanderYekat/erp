...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстОшибкиОНеаккредитованномУЦ = СтрШаблон(
		...
		|обязательной замене.';
		|en = 'Your certificate was issued by certificate authority %1
		| that was not accredited under the new rules
		|of Federal Law No. 63 ""On digital signature""
		|(as amended on 12/27/2019 No. 476-FZ). You cannot use the certificate
		|to sign reporting from 01/01/2022 and you must replace
		|the certificate.'"),
	...
	ДокументНеБудетПринятПолужирным = Новый ФорматированнаяСтрока(
		НСтр("ru = 'Вероятно, документ не будет принят контролирующим органом.';
			|en = 'Regulatory authority might not accept the document.'"), ПолужирныйШрифт);
	...
	Элементы.ДекорацияПредупреждение.Заголовок = Новый ФорматированнаяСтрока(
		...
		|" + НСтр("ru = 'Вы уверены, что хотите продолжить отправку?';
					|en = 'Are you sure you want to continue sending?'"));
