			...
			Если НЕ ЗначениеЗаполнено(КПП) Тогда
				...
				Если НЕ ЗначениеЗаполнено(КПП) Тогда
					ТекстОшибки = НСтр("ru = 'Заполните КПП в настройках организации и повторите отправку';
										|en = 'Populate KPP in the company settings and send again'");
		...
		Если РегНомерПФРПустой Тогда 
			...
			Если НЕ ТолькоПроверка Тогда
				ТекстОшибки = НСтр("ru = 'Заполните регистрационный номер СФР (бывш. ПФР) в настройках организации';
									|en = 'Fill in the Registration number in Social Insurance Fund of Russia (former PF) in the company settings'");
				ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(ТекстОшибки);
			КонецЕсли;
		...
		ИначеЕсли НЕ ДокументооборотСКОКлиентСервер.ПроверитьРегистрационныйНомерПФР(РегНомерПФР, Истина) Тогда
			...
			Если НЕ ТолькоПроверка Тогда
				ТекстОшибки = НСтр("ru = 'Регистрационный номер СФР (бывш. ПФР) в настройках организации должен состоять из 12 цифр (ХХХ-ХХХ-ХХХХХХ)';
									|en = 'Registration number in Social Insurance Fund of Russia (former PF) in the company settings must consist of 12 digits (XXX-XXX-XXXXXX)'");
				ДлительнаяОтправкаКлиентСервер.ВывестиОшибку(ТекстОшибки);
			КонецЕсли;
