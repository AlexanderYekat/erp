...
Процедура УправлениеФормой()
	...
	Если ЭтоМягкоеПредупреждение Тогда
		Заголовок = НСтр("ru = 'Ваша лицензия на тариф ""%1"" истекла';
						|en = 'Your license for the ""%1"" service plan has expired'");
	...
	Если ЭтоМягкоеПредупреждение Тогда
		...
		Если ТарифОператораЭДО = Перечисления.ТарифыОператораЭДО.КадровыеРешения Тогда
			Предложение1 = НСтр("ru = 'Чтобы продолжить использование обмена с ФСС и ПФР, необходимо продлить лицензию.';
								|en = 'Extend the license to continue using the exchange with the SSF and PF.'") + Символы.ПС;
		Иначе
			Предложение1 = НСтр("ru = 'Чтобы продолжить использование обмена с ФНС, необходимо продлить лицензию.';
								|en = 'Extend the license to continue using the exchange with the FTS.'") + Символы.ПС;
		КонецЕсли;
	...
	Иначе
		...
		Если ЛицензияПросрочена Тогда
			Если ЗначениеЗаполнено(ТарифОператораЭДО) Тогда
				...
				Строка = Новый ФорматированнаяСтрока(НСтр("ru = 'лицензию на тариф ';
															|en = 'license for the service plan'"), СсылкаНаТариф);
		...
		Если СрокДействияКлючаПросрочен Тогда
			Строка = Новый ФорматированнаяСтрока(НСтр("ru = 'ключ myDSS';
														|en = 'myDSS key'"),,,,"https://help.astral.ru/v/147062851");
			МассивКонтроля.Добавить(Строка);
		КонецЕсли;
	...
	Если ЭтоМягкоеПредупреждение Тогда
		...
		Строка = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Подробнее о тарифе читайте ';
				|en = 'Read more about the service plan'"),
			Новый ФорматированнаяСтрока(НСтр("ru = 'здесь';
											|en = 'here'"),,,,"https://astral.ru/lp/1%D1%81-kadrovye-resheniya/"),
			".",
			Символы.ПС,
			Символы.ПС);
