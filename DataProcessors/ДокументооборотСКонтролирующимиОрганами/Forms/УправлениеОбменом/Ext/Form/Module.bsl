	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ТекстПредупреждения = ДокументооборотСКОКлиентСервер.ЗаменитьПФРиФССнаСФР(
			НСтр("ru = 'Не выбраны отправки ФСС!';
				|en = 'Sendings to FTS are not selected.'"), Истина);
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			...
			ПоказатьПредупреждение(, ?(ЭтоПодтверждениеВидаДеятельности
				...
				НСтр("ru = 'Квитанция отсутствует.';
					|en = 'The receipt is missing.'"), НСтр("ru = 'Квитанция об успехе отсутствует.';
	...
	Если СуществуетРегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний() Тогда
		СписокВариантовВыбора.Добавить("УведомленияОбУчастииВМеждународнойГруппеКомпаний",
			НСтр("ru = 'Уведомление об участии в международной группе компаний';
				|en = 'Notification of international company group membership'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетДоходыОтДеятельностиМузеяТеатраБиблиотеки() Тогда
		СписокВариантовВыбора.Добавить("ДоходыОтДеятельностиМузеяТеатраБиблиотеки",
			НСтр("ru = 'Доходы от деятельности музея, театра, библиотеки';
				|en = 'Income from the museum, theater, library operations'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетДвижениеСредствПоСчетуВБанкеЗаПределамиРФ() Тогда
		СписокВариантовВыбора.Добавить("ДвижениеСредствПоСчетуВБанкеЗаПределамиРФ",
			НСтр("ru = 'Движение средств по счету в банке за пределами РФ';
				|en = 'Cash flow on the bank account outside the Russian Federation'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетПрослеживаемыеТоварыВвоз()
		...
		СписокВариантовВыбора.Добавить("ПрослеживаемыеТоварыВвоз",
			НСтр("ru = 'Уведомление о ввозе товаров, подлежащих прослеживаемости';
				|en = 'Notification of import of goods to be traced'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетПрослеживаемыеТоварыПеремещение()
		...
		СписокВариантовВыбора.Добавить("ПрослеживаемыеТоварыПеремещение",
			НСтр("ru = 'Уведомление о перемещении товаров, подлежащих прослеживаемости';
				|en = 'Notification of transfer of goods to be traced'"));
	КонецЕсли;
	...
	Если СуществуетРегламентированныйОтчетПрослеживаемыеТоварыОстатки()
		...
		СписокВариантовВыбора.Добавить("ПрослеживаемыеТоварыОстатки",
			НСтр("ru = 'Уведомление об остатках товаров, подлежащих прослеживаемости';
				|en = 'Notification of balance of goods to be traced'"));
	КонецЕсли;
...
Процедура ВыгрузитьСодержимоеОтправкиФСРАР(РежимВыгрузки)
	...
	Если КоличествоВыделенныхСтрок <= 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбраны отправки в Росалкогольтабакконтроль!';
										|en = 'Sendings to Federal Service for Alcohol and Tobacco Market Regulation are not selected.'"));
		Возврат;
	КонецЕсли;
...
Функция СодержимоеОтправкиФТСВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФТС, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 1 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ru = 'Квитанция';
									|en = 'Receipt'") + " " + СсылкаНаОбъектОтправкиФТС["ИмяФайлаВыгрузки"] + ".bin";
	ИначеЕсли РежимВыгрузки = 2 Тогда
