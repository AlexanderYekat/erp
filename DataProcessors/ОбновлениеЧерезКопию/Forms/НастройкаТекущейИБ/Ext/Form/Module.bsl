...
&НаСервере
Функция ЕстьНастройкаТранспортаОбменаОбновления(ВключитьРегламентноеЗадание = Ложь)
	...
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	РегистрСведений.НастройкиТранспортаОбменаДанными.Корреспондент
	|ГДЕ
	|	РегистрСведений.НастройкиТранспортаОбменаДанными.Корреспондент = &Корреспондент";
	
	РезультатЗапроса = Запрос.Выполнить();
	ЕстьНастройкаТранспорта = (НЕ РезультатЗапроса.Пустой());
	
	Если ВключитьРегламентноеЗадание Тогда
...
Процедура НастроитьТранспортОбменаВыгрузки()
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеНастройкиТранспортаСообщений", ЭтотОбъект);
	
	ОбменДаннымиКлиент.ОткрытьФормуЗаписиРегистраСведенийПоОтбору(Отбор, ЗначенияЗаполнения,
		"НастройкиТранспортаОбменаДанными", ЭтаФорма, , , ОписаниеОповещения);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеНастройкиТранспортаСообщений(Результат, ДополнительныеПараметры) Экспорт
	
	Если НЕ ЕстьНастройкаТранспортаОбменаОбновления(Истина) Тогда
