...
Процедура ДобавитьВЖурналЗаписьОбОшибкеИСообщитьПользователю(ПредставлениеПроблемы, КлючДанных, ОписаниеОшибки) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Объединение задвоенных данных';
									|en = 'Объединение задвоенных данных'", ОбщегоНазначения.КодОсновногоЯзыка()),
	...
	Для Каждого ОписаниеРегистраСведений Из ДанныеДляОбъединения.Строки Цикл
		...
		ОбщегоНазначенияКлиентСервер.Проверить(ЗначениеЗаполнено(ИмяИзмерения),
			СтрШаблон(НСтр("ru = 'Не найдено измерение %1 у регистра сведений %2';
							|en = 'Не найдено измерение %1 у регистра сведений %2'"), ПроверяемоеИмя, ИмяРегистра),
...
Процедура ЗаполнитьКолонкиДеревьевСравнения(Форма, КоличествоДублей, ИменаДеревьев = "ДеревоСравнения") Экспорт
	...
	Для каждого ИмяДерева Из МассивИменДеревьев Цикл
		...
		Для Индекс = 0 По КоличествоДублей - 1 Цикл
			ИндексРеквизита = УдаляемыеРеквизиты.Найти(ИмяДерева + ".Поле" + Индекс);
			Если ИндексРеквизита = Неопределено Тогда
				Реквизит  = Новый РеквизитФормы("Поле" + Индекс, Новый ОписаниеТипов(), ИмяДерева);
				ДобавляемыеРеквизиты.Добавить(Реквизит);
			Иначе
				УдаляемыеРеквизиты.Удалить(ИндексРеквизита);
			КонецЕсли; 
			
			ИндексРеквизита = УдаляемыеРеквизиты.Найти(ИмяДерева + ".ПолеПометка" + Индекс);
			Если ИндексРеквизита = Неопределено Тогда
				Реквизит  = Новый РеквизитФормы("ПолеПометка" + Индекс, Новый ОписаниеТипов("Булево"), ИмяДерева);
				ДобавляемыеРеквизиты.Добавить(Реквизит);
			Иначе
				УдаляемыеРеквизиты.Удалить(ИндексРеквизита);
			КонецЕсли; 
			
			ИндексРеквизита = УдаляемыеРеквизиты.Найти(ИмяДерева + ".ДоступенДляВыбора" + Индекс);
			Если ИндексРеквизита = Неопределено Тогда
				Реквизит  = Новый РеквизитФормы("ДоступенДляВыбора" + Индекс, Новый ОписаниеТипов("Булево"), ИмяДерева);
				ДобавляемыеРеквизиты.Добавить(Реквизит);
			Иначе
				УдаляемыеРеквизиты.Удалить(ИндексРеквизита);
			КонецЕсли; 
	...
	Для каждого ЭлементКоллекции Из Коллекция Цикл
		...
		Если СтрокаОбрабатывается Тогда
			...
			Для ИндексДубля = 0 По НайденныеДубли.Количество() - 1 Цикл
				ЗначениеПоля = ЭлементКоллекции["Поле" + ИндексДубля];
				Если Не ЗначениеЗаполнено(ЗначениеПоля) Тогда
					Продолжить;
				КонецЕсли; 
				Если ЭлементКоллекции["ПолеПометка" + ИндексДубля] Тогда
				...
				Иначе
					ЭлементКоллекции["ДоступенДляВыбора" + ИндексДубля] = Истина;
				КонецЕсли;
