...
Функция СформироватьПечатнуюФормуСправкиРасчета(Документ, ОбъектыПечати, ПараметрыПечати)
	...
	ОбластьШапкиСправки.Параметры.Документ = Документ;
	...
	ВыборкаПоОС = ДанныеДокумента.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	...
	Пока ВыборкаПоОС.Следующий() Цикл
		...
		ВыборкаПоДатам = ВыборкаПоОс.Выбрать(ОбходРезультатаЗапроса.Прямой);
		...
		Пока ВыборкаПоДатам.Следующий() Цикл
			...
			Если ЭтоПерваяСтрокаПоОС Тогда
				СуммаДолгаБезНДС = ВыборкаПоДатам.ПриведеннаяСтоимость;
				ПредыдущаяДата = НачалоДня(Документ.Дата);
				ЭтоПерваяСтрокаПоОС = Ложь;
			КонецЕсли;
			
			ДнейДолга = (ВыборкаПоДатам.Дата - ПредыдущаяДата) / 86400; 
			ИтогоДнейДолга = ИтогоДнейДолга + ДнейДолга;
			ПредыдущаяДата = ВыборкаПоДатам.Дата;
			...
			СуммаДолгаБезНДС = Макс(СуммаДолгаБезНДС - ВыборкаПоДатам.СуммаПлатежаБезНДС+ВыборкаПоДатам.Проценты, 0);
...
Функция ПолучитьДанныеСправкиРасчета(Документ, ДанныеДляПечати) 
	...
	|	ВТ_ДанныеШапки.ПриведеннаяСтоимость КАК ПриведеннаяСтоимость,
	|	ЕСТЬNULL(ВТ_ДанныеГрафиков.Дата, ДАТАВРЕМЯ(1,1,1)) КАК Дата,
