...
Процедура ЗафиксироватьСозданныеПартииОпераций(ПараметрыПланирования, МенеджерВременныхТаблиц)
	...
	|	&ТекстЗапросаКоэффициентОперации,
	...
	|	ПО ПартииОпераций.Этап = Очередь.Этап
	|		И ПартииОпераций.Операция = Очередь.Операция
	|		И ПартииОпераций.ИдентификаторОперации = Очередь.ИдентификаторОперации
	|
	|УПОРЯДОЧИТЬ ПО
	|	Выполнено УБЫВ
	|;
	...
	Запрос.Текст = СтрЗаменить(
		Запрос.Текст,
		"&ТекстЗапросаКоэффициентОперации",
		ТекстЗапросаКоэффициентОперации("Очередь"));
...
Функция ТекстЗапросаКоэффициентОперации(ИмяТаблицы)
	
	ТекстЗапроса = "%1.Требуется / (%1.Этап.КоличествоУпаковокПлан * ЕСТЬNULL(&КоэффициентУпаковки, 1)) КАК Коэффициент";
	
	ТекстЗапросаКоэффициентУпаковки = Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
		"%1.Этап.УпаковкаПлан",
		"%1.Этап.ПартияПроизводства.ОсновноеИзделиеНоменклатура");
	...
	Возврат СтрШаблон(ТекстЗапроса, ИмяТаблицы);
