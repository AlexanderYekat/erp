...
Процедура ВыгрузитьУстановленныеЦены(Команда)
	...
	Иначе
		...
		ПараметрыОжидания.ОповещениеПользователя.Пояснение = НСтр("ru = 'Текущие цены товаров выгружены на торговую площадку.';
																	|en = 'The current item prices are exported to the trading platform.'");
		ПараметрыОжидания.ОповещениеПользователя.Картинка  = БиблиотекаКартинок.ЛоготипЯндексМаркет;
...
Процедура ЗаполнитьИнформациюВПодсказках()
	...
	Элементы.ПодсказкаПросмотрИРедактированиеАссортимента.Заголовок = 
		НСтр("ru = 'Выгрузка товарного каталога осуществляется в четыре этапа:
				   ...
				   |	Step 1. Get recommended SKUs on Yandex for your items. Add the items to publish to the table below and wait for the ""Recommendation is received"" status. You can use the base price list to fill the product range data.
				   |	Step 2. Make sure the information from Yandex.Market matches your product range. Go to the ""Approve recommendations"" tab and review the received information. Approve recommendations that you think are correct. Reject incorrect recommendations.
				   |	Step 3. If necessary, process items with issues on the ""Correct errors when publishing"" tab.
				   |	Step 4. Wait for the item moderation results. Processing may take up to two days. The ""Moderation completed"" publication status means that you can proceed to set prices.'");
