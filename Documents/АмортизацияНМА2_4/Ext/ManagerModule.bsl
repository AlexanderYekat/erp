...
Процедура ТекстЗапросаПараметрыРасчетаАмортизации(СписокЗапросов, ПараметрыРасчета)
	...
	|			ТОГДА ЕСТЬNULL(КОНЕЦПЕРИОДА(ПараметрыАмортизацииНМАУУ.ДатаПоследнегоИзменения, МЕСЯЦ), ДАТАВРЕМЯ(1,1,1))
	...
	|			ТОГДА ЕСТЬNULL(КОНЕЦПЕРИОДА(ПараметрыАмортизацииНМАБУ.ДатаПоследнегоИзменения, МЕСЯЦ), ДАТАВРЕМЯ(1,1,1))
	|		ИНАЧЕ ДАТАВРЕМЯ(1,1,1)
	...
	|ВЫБРАТЬ
	|	СписокНМА.НематериальныйАктив КАК НематериальныйАктив,
	|	ЕСТЬNULL(СУММА(ВыработкаУУ.Количество), 0) КАК ОбъемВыработкиУУ,
	|	ЕСТЬNULL(СУММА(ВыработкаБУ.Количество), 0) КАК ОбъемВыработкиБУ
	|ПОМЕСТИТЬ ВыработкаНМА_НаДатуПоследнегоИзменения
	|ИЗ
	|	СписокНМА_Выработка КАК СписокНМА
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ВыработкаНМА КАК ВыработкаУУ
	|		ПО ВыработкаУУ.НематериальныйАктив = СписокНМА.НематериальныйАктив
	|			И ВыработкаУУ.Период <= СписокНМА.ДатаПоследнегоИзмененияУУ
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ВыработкаНМА КАК ВыработкаБУ
