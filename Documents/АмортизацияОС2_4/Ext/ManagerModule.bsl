		...
		Если ДанныеСтроки.ЭтоИзменения Тогда
			...
			Если ТипЗнч(ДанныеСтроки.Регистратор) = Тип("ДокументСсылка.ПереоценкаОС2_4") Тогда
				...
				Если ДанныеСтроки.СтоимостьРегл <> 0 Тогда
					...
					ЗначенияПараметров.АмортизацияРегл = Окр(ДанныеСтроки.АмортизацияРегл * Коэффициент, 2);
					ЗначенияПараметров.АмортизацияЗаМесяцРегл = Окр(ЗначенияПараметров.АмортизацияЗаМесяцРегл * Коэффициент, 2);
				КонецЕсли; 
				
				Если ДанныеСтроки.СтоимостьПР <> 0 Тогда
					Коэффициент = ДанныеСтроки.СтоимостьПРИзменение / ДанныеСтроки.СтоимостьПР;
					ЗначенияПараметров.СтоимостьПР = ДанныеСтроки.СтоимостьПРИзменение;
					ЗначенияПараметров.АмортизацияПР = Окр(ДанныеСтроки.АмортизацияПР * Коэффициент, 2);
					ЗначенияПараметров.АмортизацияЗаМесяцПР = Окр(ЗначенияПараметров.АмортизацияЗаМесяцПР * Коэффициент, 2);
				КонецЕсли; 
