...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновляет документы ""Ввод остатков внеоборотных активов"":
								  ...
								  |- Fills the ""Record in AC"", ""Record in TA"", and ""Separate VAT accounting option"" attributes'");
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковНМАиРасходовНаНИОКР Тогда
				...
				ПараметрыУчетаПоОрганизации = Неопределено;
				Если ЗначениеЗаполнено(ДокументОбъект.Организация) Тогда
					ПараметрыУчетаПоОрганизации = УчетНДСУП.ПараметрыУчетаПоОрганизации(ДокументОбъект.Организация, ДокументОбъект.Дата); 
				КонецЕсли;
				...
				Для Каждого ДанныеСтроки Из ДокументОбъект.НМА Цикл
					...
					Если Не ЗначениеЗаполнено(ДанныеСтроки.НалогообложениеНДС)
						И ПараметрыУчетаПоОрганизации <> Неопределено Тогда
						ДанныеСтроки.НалогообложениеНДС = ПараметрыУчетаПоОрганизации.ОсновнойВидДеятельностиНДСЗакупки;
					КонецЕсли;
