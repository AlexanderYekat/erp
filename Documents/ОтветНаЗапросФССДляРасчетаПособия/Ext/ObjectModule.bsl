		...
		Если ЗначениеЗаполнено(ТрудовойДоговорДатаНачала) И ЗначениеЗаполнено(ТрудовойДоговорДатаОкончания) Тогда
			...
			Если Не УчетПособийСоциальногоСтрахования.ИспользуетсяУсловиеИсчисления(ЭтотОбъект, "46") Тогда
				Текст = НСтр("ru = 'ERR_PR_1007: При указании срочного трудового договора необходимо заполнить условия исчисления: код 46';
							|en = 'ERR_PR_1007: When you specify the fixed-term employment contract, fill calculation conditions: code 46'");
	...
	Если ДанныеВходящегоЗапроса <> Неопределено Тогда
		Если ЗначениеЗаполнено(ДанныеВходящегоЗапроса.ПервыйРасчетныйГодВидЗаработка)
			...
			Текст = НСтр("ru = 'Изменен вид заработка первого года по сравнению со входящим запросом.';
						|en = 'Изменен вид заработка первого года по сравнению со входящим запросом.'");
		...
		Если ЗначениеЗаполнено(ДанныеВходящегоЗапроса.ВторойРасчетныйГодВидЗаработка)
			...
			Текст = НСтр("ru = 'Изменен вид заработка второго года по сравнению со входящим запросом.';
						|en = 'Изменен вид заработка второго года по сравнению со входящим запросом.'");
...
Процедура ПроверитьСведенияОбОтпуске(Отказ)
	...
	Если ЗначениеЗаполнено(ОтпускПоУходуДата)
		...
		Текст = НСтр("ru = 'Дата приказа об отпуске должна предшествовать дате начала отпуска (ERR_PR_1018)';
					|en = 'The date of the leave order must precede the leave start date (ERR_PR_1018)'");
