...
Функция ОтгрузкаТоваровJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	Иначе
		ОбщегоНазначенияИСКлиентСервер.ДобавитьТекстОшибки(
			СообщениеJSON,
			СтрШаблон(
				НСтр("ru = 'Неизвестный тип операции Уведомление об отгрузке маркированныхтоваров ИСМП: %1';
					|en = 'Неизвестный тип операции Уведомление об отгрузке маркированныхтоваров ИСМП: %1'"), Шапка.Операция));
	КонецЕсли;
