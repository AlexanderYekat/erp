...
Функция ДанныеЗаполненияНакладной(МассивСсылок, СвойстваЗаказов = Неопределено) Экспорт
	...
	|										КОГДА ЗаказДавальца2_5.БанковскийСчет <>
	...
	|	ВЫБОР
	|		КОГДА КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВЫБОР
	|										КОГДА ЗаказДавальца2_5.БанковскийСчетКонтрагента <>
...
Процедура ЗаполнитьПоОстаткамЗаказов(ДанныеОтбора, Таблица, МассивЗаказов, ПараметрыЗаполнения) Экспорт
	...
	СтруктураПересчета = ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(ПараметрыЗаполнения);
	СтруктураПересчета.Вставить("СтавкаНДС", ПараметрыЗаполнения.СтавкаНДС);
	...
	Пока ВыборкаПоОстаткам.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаТаблицы, ВыборкаПоОстаткам);
		
		РеквизитыЗаказов.Сбросить();
		РеквизитыЗаказов.НайтиСледующий(СтрокаТаблицы.ЗаказДавальца, "ЗаказДавальца");
		...
		Если РеквизитыЗаказов.ПересчитатьВВалютуДокумента Тогда
			...
			Если КурсВалюты.Количество() = 1
			   И КурсНовойВалюты.Количество() = 1 Тогда
				...
				СтрокаТаблицы.Цена = РаботаСКурсамиВалютУТ.ПересчитатьПоКурсу(
										СтрокаТаблицы.Цена, КурсВалюты[0], КурсНовойВалюты[0]);
