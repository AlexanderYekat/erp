...
Функция РассчитатьСуммыНУ(Период, КурсНаДату, ЗаписьНУ, Оплата = Неопределено)
	...
	НачальныйОстатокСуммаНУ = ЗаписьНУ.СуммаНУОстаток - Окр(Оплата.СуммаОборот * КурсНаДату, 2);//в регистр еще не записана курсовая разница
	...
	Если НачальныйОстатокСумма <> 0 Тогда
		...
		Если Оплата.СуммаОборот <> 0 Тогда
			КурсоваяРазницаОплаты = Окр(Оплата.СуммаОборот * (КурсОстаткаНУ - КурсНаДату), 2);
		КонецЕсли;
...
Процедура ЗаполнитьДвиженияНУФинансовыхИнструментов(Организация, Период, ХозяйственнаяОперация, ДатаНачалаПересчета = Неопределено, Регистратор = Неопределено) Экспорт
	...
	|	ПО КурсыВалют.Валюта = ТаблицаПериодов.ВалютаВзаиморасчетов
	...
	|	Расчеты.Сумма КАК Сумма,
	|	Расчеты.СуммаРегл КАК СуммаРегл,
	|	ЕСТЬNULL(КурсыВалют.КурсЧислитель,1) КАК КурсЧислительРегл,
	|	ЕСТЬNULL(КурсыВалют.КурсЗнаменатель,1) КАК КурсЗнаменательРегл
	...
	|		Расчеты.РасчетныйДокумент КАК РасчетныйДокумент,
	|		Расчеты.Сумма КАК Сумма,
	|		ВЫРАЗИТЬ(Расчеты.Сумма
	|			* Расчеты.КурсЧислительРегл / Расчеты.КурсЗнаменательРегл КАК ЧИСЛО(31,2)) КАК СуммаРегл
	|	ИЗ
