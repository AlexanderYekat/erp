...
Функция ВывестиДетальныеЗаписи(ДанныеПоОбъектуГруппировки, ДанныеВывода)
	...
	Валюта = СтрШаблон(НСтр("ru = 'в валюте';
							|en = 'in currency'", ОбщегоНазначения.КодОсновногоЯзыка()) + " %1", ДанныеПоОбъектуГруппировки.ПредставлениеВалюта);
	...
	Если НастройкиПечати.ДебетКакДолг ИЛИ ЭтоФинансовыеИнструменты Тогда
		...
		ОбластьМакетаШапка.Параметры.Кредит = НСтр("ru = 'Аванс';
													|en = 'Advance payment'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ЕстьВводОстатков(ТаблицаРезультатаСКД) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		|Reconciliation for such period is incorrect. The start of the reconciliation period must be after entering the balance. 
		|Set the start of the reconciliation period to %1.'");
