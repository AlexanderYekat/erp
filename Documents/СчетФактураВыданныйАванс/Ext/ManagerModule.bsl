...
Функция ЕстьСчетаФактурыКОформлению(Параметры) Экспорт
	...
	Если Параметры.Свойство("ЗакрытиеМесяца") Тогда
		ОтборАвансов.Вставить("НачалоПериода", НачалоКвартала(Параметры.НачалоПериода));
		ОтборАвансов.Вставить("КонецПериода", КонецКвартала(Параметры.КонецПериода));
	Иначе
