...
Процедура ЗаполнениеТребованийОснований(Параметры = Неопределено) Экспорт
	...
	Исключение
		Ошибка = ИнформацияОбОшибке();
		СтрОш = НСтр("ru = 'Ошибка при заполнении документов-оснований требования';
					|en = 'Ошибка при заполнении документов-оснований требования'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(СтрОш, УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
КонецПроцедуры
