...
Процедура ЗагрузитьСДиска(Команда)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ПараметрыЗагрузки.Диалог.Заголовок = НСтр("ru = 'Выберите формализованный документ xml';
													|en = 'Выберите формализованный документ xml'");
		ПараметрыЗагрузки.Диалог.Фильтр = НСтр("ru = 'Файлы xml (*.xml;*.xml)|*.xml;*.xml|Все файлы (*.*)|*.*';
												|en = 'Файлы xml (*.xml;*.xml)|*.xml;*.xml|Все файлы (*.*)|*.*'");
		ФайловаяСистемаКлиент.ЗагрузитьФайл(ОписаниеОповещения, ПараметрыЗагрузки);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОткрытьКонтейнерДокумента(Команда)
	...
	Иначе
		Сообщение = НСтр("ru = 'Нет данных для открытия транспортного контейнера электронного документа.';
						|en = 'There is no data required to open the electronic document transport container.'");
...
Процедура ПроверкаСуществующегоДокумента()
	Если ЗначениеЗаполнено(СуществующийДокумент) Тогда
		...
		ТекстВопроса = СтрШаблон(НСтр("ru = 'Такой документ уже был загружен ранее. %1Открыть его?';
										|en = 'Такой документ уже был загружен ранее. %1Открыть его?'"), Символы.ПС);
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("Открыть", НСтр("ru = 'Да, открыть';
										|en = 'Да, открыть'"));
		Кнопки.Добавить("Отмена", НСтр("ru = 'Нет, отменить загрузку';
										|en = 'Нет, отменить загрузку'"));
	...
	Если ТаблицаОшибок.Количество() = 0 Или ЗагрузитьПринудительно Тогда
		...
		ТекстПредупреждения = СтрШаблон(
			НСтр("ru = 'В исходящем документе %1 был загружен сторонний файл. Имя файла %2';
				|en = 'В исходящем документе %1 был загружен сторонний файл. Имя файла %2'"),
