...
Процедура НачатьЭтап(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОтметитьНачалоВыполненияЗавершение", ЭтотОбъект, Новый Структура());
...
&НаКлиенте
Процедура ОтметитьНачалоВыполненияЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	
	Если Ответ = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ОбработатьУстановкуСтатусаИВыполнитьДополнительныеДействия(Ответ, ДополнительныеПараметры);

КонецПроцедуры

&НаСервере
Процедура ОбработатьУстановкуСтатусаИВыполнитьДополнительныеДействия(Ответ, ДополнительныеПараметры)
		
	НачатоВыполнениеЭтапа = Истина;
	ОбработатьНачалоВыполненияЭтапа();
	
	Если Ответ.ЗаполнитьОбеспечение Тогда
		
		Результат = ОбеспечениеУстановитьДействие(Ответ.КодДействия, Ложь);
		НуженВопрос = Результат.ЗадаватьВопросОЗаполненииЦелымиУпаковками Или Результат.СнятьФлагОтгружатьОднойДатой;
		Если НуженВопрос Тогда // авто-ответ
			Если Объект.НеОтгружатьЧастями Тогда
				ОбеспечениеУстановитьДействиеПослеВопроса("ОТГРУЖАТЬ_ОДНОЙДАТОЙ_ДРОБНЫМИ_УПАКОВКАМИ", Результат);
			Иначе
				ОбеспечениеУстановитьДействиеПослеВопроса("ДРОБНЫМИ_УПАКОВКАМИ", Результат);
			КонецЕсли;
		КонецЕсли;
		
		Если Ответ.ЗаменитьНаАналоги Тогда
			ЗаменитьНаАналоги(, Ответ.КодДействияАналоги, Ложь);
		КонецЕсли;
		
	КонецЕсли;
	
	Если Ответ.НазначитьИсполнителя Тогда
		Объект.Исполнитель = Ответ.Исполнитель; // проверка исполнителя не нужна
	КонецЕсли;
...
Процедура ОтразитьКоличествоФактВТЧПослеВопроса(Результат, ДополнительныеПараметры) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Операция завершена';
										|en = 'Операция завершена'"),,,БиблиотекаКартинок.Информация32);
...
//		ИЛИ СтруктураРеквизитов.Свойство("ПобочныеИзделия")
	...
	Если СтруктураРеквизитов.Свойство("КоличествоУпаковокФакт")
		ИЛИ СтруктураРеквизитов.Свойство("Статус")
		ИЛИ СтруктураРеквизитов.Свойство("СостояниеОпераций")
		ИЛИ Инициализация Тогда
		
		Элементы.Производство_ОтразитьБрак.Видимость = Форма.ЕстьПравоНаИзменение
			И Объект.НомерСледующегоЭтапа <> 0
			И Объект.Статус <> СтатусЗавершен()
			И (Объект.КоличествоУпаковокФакт > 0 ИЛИ Форма.ЕстьБракВОперациях);
...
Процедура ЗаменитьНаАналоги(
	...
	Если ВыводитьСообщениеОРезультате Тогда
		ТекстСообщения  = СтрШаблон(НСтр("ru = 'Измененных строк: %1';
										|en = 'Измененных строк: %1'"), КоличествоПроизведенныхЗамен);
