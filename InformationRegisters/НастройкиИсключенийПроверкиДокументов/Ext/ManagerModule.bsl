...
Функция ИсключаемыеТипыДокументовДляПроверки(ТолькоБезВозможностиИзменения = Ложь) Экспорт
	...
	Если Не ТолькоБезВозможностиИзменения Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	Данные.ВидДокумента.Имя КАК ИмяДокумента,
		|	ТИПЗНАЧЕНИЯ(Данные.ВидДокумента.ЗначениеПустойСсылки) КАК Тип
		|ИЗ
		|	РегистрСведений.НастройкиИсключенийПроверкиДокументов КАК Данные
		|ГДЕ
		|	НЕ Данные.Проверяется";
		
		МассивВозврата = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Тип");
...
Функция ИсключаемыеДокументыДляПроверки(ТолькоБезВозможностиИзменения = Ложь) Экспорт
	...
	Если Не ТолькоБезВозможностиИзменения Тогда
		...
		МассивВозврата = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ИмяДокумента");
